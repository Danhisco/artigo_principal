...
```{r}
rmarkdown::render("1_to_compile_dissertacao_EM_USO/00_Resultados/Resultados.Rmd", 
                  output_format = "bookdown::pdf_document2")
df_files <- data.frame(
  paths = list.files(path="1_to_compile_dissertacao_EM_USO/00_Resultados/figuras",
                     pattern=".png",
                     full.names = T)
)
df_files$tamanho = lapply(df_files$paths,\(li){
  if(file.info(li)$size/(10^6)>1){
    return("grande")
  }else{
    return("pequena")
  }
...
library(magick)
a_ply(filter(df_files,tamanho=="grande"),1,\(dfi){
  img <- image_read(dfi$paths) %>% 
    image_trim() %>% 
    image_resize("50%")
  image_write(img,path=dfi$paths)
})
...
```{r}
dfpred <- adply(l_path$te,1,\(i){
    hgam <- readRDS(i)
    hgam <- hgam[[ unique( df_tabsel$modelo ) ]]
    f_dfmd2(hgam$model) %>% 
      mutate(efeito = str_extract(i,"(?<=md\\_)(.*?)(?=\\.rds)"))
  },.id=NULL)
dfpred %>% head()
dfpred$predicao <- rnorm(nrow(dfpred))
#
dfpred %>% 
  ...
  ggplot(aes(x=k_z,y=Uefeito,z=predicao)) +
  ...
  facet_grid(pert_class~efeito,scales = "fixed") +
  theme_classic()
...
```{r}
#
l_dfpred <- lapply(l_rds,readRDS) %>% 
  lapply(.,"[[","apenas fixo")
lapply(l_dfpred,\(dfi){
  dlply(dfi,"forest_succession",\(x) identical(unique(df_pred_comum$k),unique(x$k)))
})
teste <- rbindlist(l_dfpred)
dlply(teste,c("efeito","forest_succession"),\(dfi){
  identical(unique(df_pred_comum$k),unique(dfi$k))
})


lp <- lapply(l_dfpred,\(dfi){
  dfi %>% 
     ggplot(aes(x=k,y=Uref,fill=Q_0.5)) + geom_raster() + facet_grid(forest_succession~efeito,scales="free")
})
grid.arrange(grobs=lp,ncol=3)

dfpred %>% 
  filter(quantil=="50") %>%
  ggplot(aes(x=k,y=Uref,fill=predicao)) + 
  geom_raster() + 
  facet_grid(forest_succession~efeito,scales="free")
names(l_dfpred) <- str_extract(l_rds,"(?<=pred\\_)(.*?)(?=\\.rds)")
dfpred <- rbindlist(l_dfpred) %>% 
...
dfpred <- readRDS(file = "dados/csv_SoE/rds/dfpred_p_plot_figfinal.rds")
f_ggplot <- \(dff,
              vscales="fixed",
              lwidth=0.3,
              textsize=3.5
...
p <- f_ggplot(filter(dfpred,quantil=="50"),vscales = "fixed")
p2 <- f_ggplot(dfpred,vscales = "free")
p_ate1km <- f_ggplot(filter(dfpred,quantil=="50",k>=0.35),vscales = "fixed")
...
ggsave(filename="figuras/temp_figfinal/comparacao_efeitos_scalefixed_ate1km.png",
       plot = p_ate1km,width=14,height = 12)
library(magick)
vpath <- "figuras/temp_figfinal/comparacao_efeitos_scalefixed_ate1km.png"
img <- image_read(vpath) %>% 
  image_trim() %>% 
  image_resize("75%")
image_write(img,gsub("png","jpeg",vpath))
file.remove(vpath)
```
