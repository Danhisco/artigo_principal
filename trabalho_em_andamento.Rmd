...
f_ggplot <- \(dfi){
  ...
  l_p$obs_uefeito_predicoes <- dfi %>% 
    ...
    geom_point() +
    # te
    geom_ribbon(aes(ymin = logOR_te_5, ymax = logOR_te_5, 
                    fill = "Quantile range"), 
                alpha = 0.2) +
    geom_line(aes(y = logOR_te_50, color = "Median", group = SiteCode), 
              alpha = 0.5) +
    ...
    geom_ribbon(aes(ymin = logOR_s_5, ymax = logOR_s_5, 
                    fill = "Quantile range"), 
                alpha = 0.2) +
    geom_line(aes(y = logOR_s_50, color = "Median", group = SiteCode), 
              alpha = 0.5) +
...
lapply(levels(df_s$SiteCode),\(li){
  vpath <- paste0(v_path,"figuras/PIeOBS_sites/",li,".png")
  p <- filter(df_plot,SiteCode==li) %>% 
    f_ggplot()
...
f_ggplot <- \(dfi){
  dflabel <- ddply(dfi,"predicao",\(dfw){
    data.frame(
      predicao = dfw$predicao[1],
      v_acima0 = sum(dfw$logOR_pred>0)/nrow(dfw),
      v_igual0 = sum(dfw$logOR_pred==0)/nrow(dfw),
      v_abaixo0 = sum(dfw$logOR_pred<0)/nrow(dfw)) %>% 
  ...
  inner_join(dfi,dflabel) %>% 
    ...
    facet_wrap(~predicao,ncol=1,scales="free")
}
f_plotdensityplot <- \(vstring){
  ...
  p <- dfi %>% 
    ...
    f_ggplot() +
    labs(title=vstring)
  vstring <- gsub("Área per se","areaperse",vstring) %>% 
    gsub("Frag. per se","fragperse",.) %>% 
    gsub("Frag. total","fragtotal",.)
  ggsave(paste0(v_path,"figuras/density_",vstring,".png"),bg = "white")
}
