...
f_ggplot <- \(dfi){
  ggplot(dfi,aes(x=logOR_pred,fill=predicao)) +
    geom_density(alpha = 0.5) +
    geom_density(data=filter(dfi,predicao=="fixo e aleat."),alpha=0.5,
                 mapping=aes(x=logOR,fill="observado")) +
    scale_fill_manual("",values=c("fixo e aleat."="#C04000",
                               "somente fixo"="darkgreen",
                               "observado"="black"),
                      breaks = c("fixo e aleat.", 
                                 "somente fixo", 
                                 "observado")) +
    theme_minimal() +
