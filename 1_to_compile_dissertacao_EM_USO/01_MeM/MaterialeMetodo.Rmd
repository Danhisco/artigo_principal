...
FINAL DO INSERTO DE INTRODUÇÃO
...
Para finalizar a sequência epistêmica das práticas ontológicas iremos avaliar os efeitos causais da paisagem na congruência da SAD simulada. A estimativa de efeito ocorre pelo contraste de contrafactuais: área per se, paisagem aglomerada e paisagem prístina; fragmentação per se, paisagem contemporânea e paisagem aglomerada; fragmentação total, paisagem contemporânae e paisagem prístina. O efeito da paisagem será avaliado pelo log odds ratio da probabilidade da SAD simulada ter boa congruência com a SAD observada, dado um grau de limitação de dispersão per capita. A SAD simulada é comparada com a SAD observada pelo teste de Kolmogorov-Smirnov bootstrap e é contabilizado o número de SADs simuladas em que não foi possível refutar a hipótese nula, de que ambas SADs são amostras de uma mesma distribuição. A proporção de SADs simuladas com boa congruência foi usada para calcular o log odds ratio de contrafactuais em contraste.


Perguntas
Para avaliar cada efeito causal da paisagem, nós primeiro usamos as tabelas da figura X para responder às seguintes perguntas: O modelo mais plausível acumular a maior parte do peso de evidência dos dados? Esse modelo explica a maior parte da variabilidade dos dados? Seguimos avaliando os gráficos do intervalo de predição a posteriori do modelo mais plausível (gráficos figura X) para responder: Qual é o efeito causal fixo da paisagem na congruência relativa da distribuição de abundância de espécies (SAD) simulada?  E qual o efeito causal da paisagem por sítio de amostragem, e existem características comuns entre o efeito fixo e por sítio?


Questions
To assess each causal effect of the landscape, we will first use the tables in Figure X to determine the following: Is the most plausible model accumulating most of the weight? Does this model explain a significant portion of the variability in the data? Next, we will examine the posterior interval distribution in Figure X to address: What is the fixed causal effect of the landscape on the relative goodness-of-fit of the simulated species abundance distribution (SAD)? Additionally, what is the site-level causal effect of the landscape, and are there common features between the site-level and fixed effects?
...
f_plot <- \(drow){
  ...
  image_write(img,path=paste0("./figuras/exemplo_",drow$v_efeitos,".png"))
}
