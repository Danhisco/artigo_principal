...
<p>A seção dos resultados será composta de 3 seções (7.1, 7.2 e 7.3). Em 7.1 foi feita uma análise exploratoria dos dados empirícos selecionados, i.e., os dados de inventários florestais dentro do critério de inclusão e os respectivos mapas de cobertura florestal do ano mais próximo do inventário. Na subseção 7.2 será apresentado os principais resultados do estudo da análise do efeito escalar da extensão espacial da paisagem ao redor associado com o aumento da capacidade de dispersão per capita. Nessa seção 7.2 será definido a escala espacial da paisagem suficiente para simular cada um dos 20 cenários de limitação de dipersão explorados em cada sítio e paisagem hipotética. Na subseção 7.3 a análise dos efeitos explicativos da paisagem na predição da biodiversidade local é dividada em duas partes (7.3.1 e 7.3.2). Em 7.3.1 foi feito a análise exploratória da métrica de conectividade, definida como logU/U, para em cada tipo de efeito explicativo. Em 7.3.2 foi desenvolvido a descrição da congruência da SAD simulada com a SAD observada, essa seção foi dividida em duas partes: 7.3.2.1 - descrição da congruência absoluta da SAD simulada por paisagem hipotética; e 7.3.2.2 - descrição da congruência relativa da SAD simulada entre as paisagens hipotéticas em contraste no efeito explicativo.</p>

## 7.1 Dados Empíricos: inventários florestais e mapa de cobertura florestal contemporâneo

<p>Nesse capítulo será apresentado as características dos inventários florestais: coordenadas geográficas, área da parcela, número de indivíduos na amostra, riqueza de espécies, ano aproximado da amostragem. Os sítios serão contabilizados segundo a classe de perturbação usado na base de dados TreeCo, que considera o histórico e tipo de perturbações na parcela (mais detalhes na seção 6.3.2).</p> 

## 7.2 Efeito escalar da dispersão 

<p>Essa seção contará com a análise exploratória e a descrição estatística dos resultados obtidos a partir da subseção “Estimativa da extensão espacial adequada”. O objetivo é determinar a extensão espacial da paisagem hipotética adequada para simular cada cenário de limitação de dispersão.</p> 
