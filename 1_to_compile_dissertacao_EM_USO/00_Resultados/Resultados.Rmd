...
#### Descrição do padrão da métrica funcional

Quando há pouca perda de habitat o log da razão da estimativa da taxa U é similar entre os 3 contraste de contrafactuais (fig. \@ref(fig:GE-contraste_taxaU)). A magnitude do log da razão da taxa U aumenta em graus de limitação de dispersão mais brandos, seguido pelos graus mais severos (REF). Nos graus de limitação de dispersão pouco severos, onde há o máximo global na estimativa da taxa U em paisagens sem perda de habitat (olhar seção ‘Efeito de Escalar’), ocorre a menor variabilidade entre os contrafactuais (fig. \@ref(fig:GE-contraste_taxaU)). Nas paisagens abaixo de 0.95, o contraste de fragmentação total pode apresentar a maior magnitude por sítio e o de área per se apresenta a menor magnitude 
...
df_quantisobs_Uefeito <- df_quantisobs_Uefeito %>% 
  mutate(`Prática Ontológica` = ifelse(`Contraste log(U/U)`=="Frag. total",
