...
```{r criacao GE dados disponiveis,eval=FALSE,echo=FALSE,include=FALSE}
...
l_p[[1]] <- df_plot |> 
  ...
  scale_fill_gradientn("% CF",colours = terrain.colors(10)) +
  theme_gray() +
...
l_p[[3]] <- df_plot |> 
  ...
  labs(x="classificação de grau de perturbação do TreeCo",
       y="countagem",subtitle="c)")
p <- arrangeGrob(grobs=l_p,nrow=1)
ggsave(filename = "./figuras/GE_dados_disponiveis.png",plot = p,
       width=12,height = 5)
```
```{r caption GE dados disponiveis}
cap <- ""
```
```{r pring GE dados disponiveis}
...
names(df) <- c("GAMM","métrica funcional\nlog(U/U)","ano obs.","coord. geo. obs.","intercepto por Site","métrica funcional\npor Site")
knitr::kable(df, 
             caption = "GAMM usados para descrever os contraste de contrafactuais\n method='REML'")
```
...
```{r}
df_tabelaSelecao <- read_csv(file="dados/csv/df_tabelaSelecao_logOR_itself.csv")
df_tabelaSelecao %>% filter(pair=="contemp-ideal") %>% select(-pair)
```
...
```{r}
df_tabelaSelecao <- read_csv(file="dados/csv/df_tabelaSelecao_logOR_itself.csv")
df_tabelaSelecao %>% filter(pair!="contemp-ideal")
```
