...
```{r criacao GE dados disponiveis,eval=TRUE,echo=FALSE,include=TRUE}
...
cap <- ""
```
```{r pring GE dados disponiveis,eval=FALSE,include=FALSE}
knitr::include_graphics(path="/home/danilo/Documentos/mestrado_Ecologia/artigo_principal/figuras/GE_dados_disponiveis.png")
```
...
cap <- ""
```
```{r figura SoE, eval=FALSE,include=FALSE}
knitr::include_graphics(path = "/home/danilo/Documentos/mestrado_Ecologia/artigo_principal/figuras/fig5_SoE.png")
```


## Práticas ontológicas em disputa coexistindo em um sistema epistêmico contrafactual baseado em indivíduos

Em ambas práticas ontológicas primeiro alguma característica da paisagem, que interpreta-se estar relacionada com a biodiversidade local, é mensurada e então é quantificado o efeito desta métrica da paisagem na biodiversidade local. 

Aqui interpretamos como métrica de influência da paisagem na biodiversidade local a razão entre a taxa U estimada nas paisagens contrafactuais em contraste. A razão da taxa U é o risco relativo de colonização de uma nova espécie na paisagem por evento de nascimento. Quanto menor a taxa U necessária para simular a riqueza observada, maior é a reposição simulada de espécies pela imigração de propágulos da paisagem ao redor para a parcela. Assim, se o log da razão das taxa U estimadas em cada paisagem contrafactual for igual a zero não há diferença entre contrafactuais com relação a reposição de espécies locais pela paisagem. Se for positivo, então a paisagem contrafactual no numerador apresenta taxa U superior; e se for negativo o contrário, o denominador apresenta valor superior. 
...
Nós quantificamos o efeito da métrica da paisagem na congruência relativa dos contrafactuais em contraste. A congruência relativa é obtida em duas partes. Primeiro, a SAD simulada é comparada com a SAD observada pelo teste de Kolmogorov-Smirnov bootstrap e é contabilizado o número de SADs simuladas com boa congruência com a SAD observada. Segundo, é calculado o log da chance de uma SAD simulada ter boa congruência para cada contrafactual e então é calculado o log da razão da chance dos contrafactuais em contraste. O log da razão da chance é usada como variável resposta de uma análise de regressão que tem a métrica funcional da paisagem como efeito fixo e aleatório, com cada paisagem livre para ter um particular efeito, e com as coordenadas espaciais do inventário e o ano do inventários como possíveis covariáveis fixas (seção 'Congruência relativa').
...
Quando há pouca perda de habitat a estimativa da taxa U não difere entre as 3 paisagens contrafactuais (fig. \@ref(fig:GE-Urep-sitio-k-land)). A maior variação entre graus de limitação de dispersão ocorre nos graus de limitação de dispersão mais brandos simulados, seguido pelos graus mais severos; nos graus de limitação de dispersão pouco severos, onde há o máximo global nas paisagens infinitas e sem perda de habitat (olhar seção ‘Efeito de Escalar’), ocorre a menor variabilidade entre tipos de paisagens hipotéticas (fig. \@ref(fig:GE-Urep-sitio-k-land)). Nas paisagens abaixo de 0.95, a paisagem hipotética contemporânea costuma apresentar estimativas superiores de taxa U em comparação com as outras duas paisagens contrafactuais (fig. \@ref(fig:GE-Urep-sitio-k-land)). Em alguns casos, a estimativa em paisagem aglomerada pode ser superior ao estimado na paisagem contemporânea (fig. \@ref(fig:GE-Urep-sitio-k-land), na 2a coluna e 4a linha e na 10a coluna e 1a linha ). Sobre a estimativa da taxa U, em resumo: i) quanto maior a perda de habitat e mais branda a limitação de dispersão, maior a potencial diferença entre as 3 paisagens contrafactuais; ii) em geral a estimativa em paisagens contemporâneas é superior; iii) em geral o estimado na paisagem aglomerada e prístina são similares, porém, para alguns sítios com pouca proporção de cobertura vegetal, o estimado em paisagem aglomerado pode ser superior. 
...
cap <- ""
```
```{r GE taxa U, include=FALSE,eval=FALSE}
knitr::include_graphics(path="/home/danilo/Documentos/mestrado_Ecologia/artigo_principal/figuras/GE_taxaU_contrastes.png")
```

#### Contraste entre conttrafactuais




Diagnóstico dos efeitos da taxa U

GRÁFICOS DESCRITIVOS DOS CONTRASTES EM FUNCAO DE P E K 

```{r moodelos estatísticos usados para descrever as métricas funcionais baseadas na taxa U, echo=FALSE,include=TRUE}
### VAI SAIR? ###
df <- data.frame(
  modelo = c("gi", "gs", 
             "gi - p * k","gs - p * k",
...
names(df) <- c("GAMM","métrica funcional","ano obs.","coord. geo. obs.","intercepto por Site","métrica por Site")
knitr::kable(df,
