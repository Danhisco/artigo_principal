...
```{r}
names(df_moranI)[c(2,5,6)] <- c("MoranI_stat","SD","pvalue")
# vvline <- df_moranI %>% filter(MoranI_stat==max(MoranI_stat)) %>% pull(k) %>% as.numeric()
df_summar <- ddply(df_moran,"modelo",\(dfi){
  dfi %>% 
    filter(MoranI_stat_res==max(MoranI_stat_res)) %>% 
    pull(k) %>% as.numeric()
})
df_sumar <- df_moran %>% 
  group_by(modelo) %>%
...
p <- df_moran %>% 
  ...
  geom_line(aes(y = MoranI_stat_res), color = "blue") +
...
ggsave("figuras/MoranI_k1ate100_PVALeSTAT.jpeg",p,
