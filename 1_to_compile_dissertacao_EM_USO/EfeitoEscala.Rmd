...
```
```{r figSI21,fig.cap=vcap,fig.width=4,fig.height=3}
...
```
```{r figSI22,fig.cap=vcap,fig.width=4,fig.height=3}
l_p <- readRDS(file="1_to_compile_dissertacao_EM_USO/03_EfeitoEscala/Rdata/l_p.rds")
l_p[[2]]
```
...
```
```{r figSI24, fig.cap=vcap,fig.height=6,fig.width=10}
...
vcap <- ""
```
```{r figUscaleObs,fig.cap=vcap,fig.height=12,fig.width=8,eval=TRUE,include=TRUE}
dfp <- df_Usoe
...
```{r cap fig Ucv}
vcap <- "Scale-dependent variation and simulation uncertainty of U-rates. (a) Coefficient of variation (CV%) of mean U across spatial scales as a function of dispersal limitation (k). (b) Within-scale and plot variability, represented by the standard deviation (SD) among simulation replicates for each dispersal level and stratified by landscape side length (panels, in km)."
```
```{r fiUcv,fig.width=10,fig.height=10,fig.cap=vcap}
l_p <- list()
l_p[[1]] <- df_Usoe |>
  group_by(SiteCode,k) |>
  summarise(Ucv = sd(Umed)*100/mean(Umed)) |>
  ...
  labs(x="k",
       y="CV of mean U (%)",
       title="a) variation in mean U across spatial scales") +
  theme_classic()
l_p[[2]] <- df_Usoe |>
...
df_rep %>% 
  ...
  ggplot(aes(x=lado_factor,y=value,color=name,group=name)) +
...
"Model selection revealed that community structure (species richness, $S$, and abundance, $N$) accounted for the majority of the explained variance in congruence ($R^2_m$ decreased from 0.96 to 0.20 upon their removal; Table X). To isolate the specific contribution of dispersal limitation ($k$) and landscape scale from this dominant influence of local community parameters, we calculated predicted mean $U$-rates from the full interaction model. We achieved this by marginalizing across $S$ and $N$ (i.e., fixing these covariates at their mean values, z-score = 0), thereby effectively controlling for the confounding effects of community-level variation."
...
<!-- dfi <- df_Usoe %>% select(SiteCode,k,lado_km,Umed) -->
