...
```
```{r figSI23, fig.cap=vcap,fig.width=10,fig.height=8}
df_p <- df_Unull |> 
  arrange(S_obs) |> 
...
vcap <- ""
```
```{r figUscaleObs,fig.cap=vcap,fig.height=12,fig.width=10,eval=TRUE,include=TRUE}
dfp <- df_Usoe
...
```
```{r fiUcv,fig.width=6,fig.height=10,fig.cap=vcap}
l_p <- list()
l_p[[1]] <- df_Usoe |>
  group_by(SiteCode,k) |>
  summarise(Ucv = sd(Umed)*100/mean(Umed)) |>
...
#-----------------------------------------------------------------------------
#-------------------------- U acumulado---------------------------------------
#dfi <- df_soe %>% filter(k_factor=="0.65")
