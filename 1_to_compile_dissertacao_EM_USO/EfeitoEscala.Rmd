...
f_prow <- \(dfi){
  vl <- c(0.75,0.90)
  names(vl) <- gsub("0\\.","l",vl)
  lt <- lapply(vl,\(li) dfi[which.min(abs(li - dfi$Ucsum)),c("lado_km","Ucsum")] )
  as.data.frame(lt)
}
dfp <- as.data.table(df_Usoe)
dfp <- dfp[,f_prow(.SD),by=c("SiteCode","k")] %>% 
  pivot_longer(cols = starts_with("l"),
               names_to="limiar",
               values_to = "L") %>% 
  mutate(limiar=factor(limiar,
                       levels = c("l75","l9","l95"),
                       labels = c("l75","l90","l95"))
         )

dfp %>% 
  ggplot(aes(x=limiar, y=L)) +
  geom_boxplot() +
  geom_jitter() +
  facet_wrap(~k,ncol=5)
```
...
Fig SI2.5: U (y) ~ scale (x) * k (color) | Plot (faceta)
 

? Fig SI2.6: U (y) ~ scale (x) * k (color) | Plot (faceta) ?
...
```{r figura 2+1 relacoes nao triviais da riqueza e do numero de individuos,eval=FALSE,include=FALSE}
df_Unull |> 
  group_by(SiteCode,Ntotal,S_obs,k,lado_km) |> 
  summarise(Umed = mean(U)) |> 
  # pivot_longer(Ntotal:S_obs) |> 
  # filter(name=="S_obs") |> 
  ggplot(aes(x=S_obs,y=Umed)) +
  geom_point(aes(color=Ntotal)) +
  geom_smooth(method="gam",se=F,alpha=0.6) +
  facet_wrap(~k)
```
