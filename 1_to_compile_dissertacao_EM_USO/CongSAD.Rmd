---
title: Empirical validation of MNEE
output:
...
```
```{r figCong,fig.cap=vcap,fig.width=12}
...
```
```{r fig-implicacoes-cong,fig.cap=vcap,fig.width=12}
library(sf)
library(patchwork)
...
knitr::include_graphics(path="1_to_compile_dissertacao_EM_USO/09_SI/figuras_SI/mapas_cobertura/selecionadas_TRUE.jpeg")
```
...
knitr::include_graphics(path="1_to_compile_dissertacao_EM_USO/09_SI/figuras_SI/mapas_cobertura/selecionadas_FALSE.jpeg")
```
...
```{r gráfico diagnóstico padrão, echo=FALSE,include=TRUE,eval=TRUE,fig.width=12, cache=TRUE}
df_tabsel <- read_csv(file="1_to_compile_dissertacao_EM_USO/09_SI/RDS/tabsel_cong.csv") %>% 
  mutate(across(where(is.numeric), ~round(.x,2)),
         modelo=f_label(modelo))
# carregar 
