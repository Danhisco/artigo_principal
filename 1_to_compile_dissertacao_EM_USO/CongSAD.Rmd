---
title: Validação empírica de MNEE e suas implicações para a estimatva de efeitos da
  paisagem
output:
...
```{r gráfico diagnóstico padrão, echo=FALSE,include=TRUE,eval=TRUE,fig.width=12}
df_tabsel <- read_csv(file="1_to_compile_dissertacao_EM_USO/09_SI/RDS/tabsel_cong.csv") %>% 
  mutate(across(where(is.numeric), ~round(.x,2)),
         modelo=f_label(modelo))
# carregar 
