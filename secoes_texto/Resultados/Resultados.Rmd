...
df_newpred <- read_csv("./dados/csv/df_ip_diffS.csv")
...
__tabela \ref{ts_prcong}__ GAMMs usados para descreever a probabilidade de congruência com o observado em cada paisagem hipotética 

```{r ts  Pr(Cong),fig.cap="\\label{ts_prcong}ts_prcong"}
df_tab <- read_csv("./dados/csv/ts_PrCong_LandHyp_GAMMs.csv")
df_tab |> 
  gt()
```

```{r ip logOR_prcong ~ p * land_hyp, fig.cap="\\label{ip_logOR_prcong}ip_logOR_prcong"}
l_df <- list()
...
f_typetopair <- \(df){
  ...
  f_adply <- \(df_i1r){
    ...
    cbind(select(df1,1:SiteCode),df_return) |> 
...
f_plot <- \(df,name_df,justmedian=T){
  f_ggplot <- \(rvar){
