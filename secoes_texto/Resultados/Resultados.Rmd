...
```{r ip prcong, fig.cap="\\label{ip_prcong}ip_prcong"}
df_ip <- read_csv("./dados/csv/df_ip_prcong_landhpyp_k.csv")
f_typetopair <- \(df){
  ...
  f_adply <- \(df_i1r){
    ...
    cbind(df |> select(1:SiteCode),df_return) |> 
