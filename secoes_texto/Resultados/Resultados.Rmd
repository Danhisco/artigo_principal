...
Observamos que, exceto em graus de limitação de dispersão pouco severos onde há o máximo global na estimativa da taxa U em paisagens com maior extensão espacial (lado de 16 km) e sem perda de habitat (Fig A1 2a e 2b), a extensão espacial adequada tende a aumentar com a redução da limitação de dispersão (Figura 3). Nos graus de limitação de dispersão em que isso não foi observado há pouca variação na média da taxa U entre escalas e sempre grande variação entre réplicas de uma mesma bateria de simulações (Fig A1 2a, A1 2b e A1 4b). Quando a proporção de propágulos que permanece até a vizinhança imediata do progenitor (k) varia entre 0.99 e 0.90, a extensão espacial da paisagem adequada é de 1 km de lado (Figura 3). Para k entre 0.80 e 0.55 não é possível determinar uma tendência de redução da média da estimativa de U (Figura 3); a variação entre escalas para um mesmo sítio é baixa (detalhes em Apêndice: Efeito de Escalar). Nesses graus de limitação de dispersão, o aumento do número de espécies per capita pode aumentar a divergência da média geral de estabilidade e apresentar padrões não lineares (Figuras A1 2 e A1 3). Em k entre 0.50 e 0.35 a extensão adequada foi de 1 km de lado; entre 0.30 e 0.20, 2 km de lado; e entre 0.15 e 0.05, 4 km de lado (Figura A1 5 e 6). Pressuposto que a extensão espacial adequada para simular k entre 0.80 e 0.55 pode ser obtida interpolando as estimativas, então nesses graus de limitação de dispersão a extensão suficiente é a de 1 km de lado da paisagem ao redor.
...
## Descrição do padrão observado[f]

Na figura \@ref(fig:GE-nCong) há os gráficos de congruência da SAD simulada e observada (número de SADs simuladas congruentes com a SAD observada) em cada sítio de amostragem. No eixo x há o grau de limitação de dispersão per capita (k, proporção de propágulos que permanece até a vizinhança imediata da progenitora). O fundo dos painéis está colorido pela proporção de cobertura vegetal (p). Os painéis estão agrupados pela classe de estado sucessional da base de dados TreeCo. No canto inferior direito a distribuição de sítios em função de p e da classe de sucessão[g]. É possível observar, em alguns graus de limitação de dispersão, diferença no predito entre paisagens hipotéticas em paisagens com cobertura vegetal abaixo de 0.92 e pode aumentar com a redução da cobertura vegeta[h]l (fig.  \@ref(fig:GE-nCong)).  O predito em paisagens idealizadas e sem fragmentação apresenta menor divergência.  Em geral, o predito em paisagens contemporâneas pode apresentar congruência melhor ou pior, em relação às outras duas paisagens hipotéticas, em graus de limitação de dispersão mais brandos[i]. Existe muita diversidade entre sítios, mas podemos classificar os padrões de congruência com o observado em tipos: sempre [j]bom ou ruim; ruim ou bom em graus de limitação de dispersão brandos; ruim ou bom em graus de limitação de dispersão intermediários. Ou seja, observamos variabilidade[k] em todo o gradiente de limitação de dispersão simulada, com maior variabilidade em graus brandos de limitação de dispersão, seguido de graus severos, e menor variabilidade em graus de limitação intermediários. A classe sucessional do sítio de amostragem parece aumentar a variabilidade no padrão de congruência: em sítios primary e secondar[l]y, paisagens contemporâneas parecem ser tão boas quanto ruins[m]; em sítios secondary, paisagens idealizadas tendem a ser melhores. [n]
  

Especulamos que parte da diversidade observada[o] pode estar relacionada com a distribuição espacial dos sítios. As paisagens contemporâneas sugerem que algumas parcelas  foram amostradas em paisagens locais com alto grau de sobreposição espaço-temporal (Fig. SI 1). [p]
Além disso, possíveis determinantes da diversidade local observada na Floresta Atlântica podem estar associados às variáveis preditoras empíricas (p e classe de sucessão) por conta da sua localidade (e.g. fig SI 2). [...[q]][r]
Na figura \@ref(fig:GE-coordenadas-preditoras) há os resultados dessa classificação visual [s]dos painéis da figura \@ref(fig:GE-nCong) e as preditoras empíricas[t], proporção de cobertura vegetal (p) e classe de sucessão do local de amostragem,  em função das coordenadas geográficas dos sítios de amostragem. As preditoras empíricas, proporção de cobertura vegetal (p) e classe de sucessão, parecem apresentar distribuição espacial não negligenciável[u] (figura \@ref(fig:GE-nCong) c:d), com agregação de paisagens com alta proporção de cobertura vegetal na região costeira[v]. 
Para realizar uma avaliação preliminar da possível influência do espaço nos padrões de congruência observados (figura \@ref(fig:GE-nCong)), fiz uma classificação visual dos sítios em dois eixos: similaridade e diferença entre os tipos de paisagens hipotéticas e congruência do modelo neutro[w] (considerando simultaneamente os 3 tipos de paisagens). Os sítios em que a paisagem contemporânea apresenta melhor ou pior congruência parecem apresentar autocorrelação positiva[x], porém as outras características da congruência com o observado são de difícil avaliação visual. 

Outro ponto importante que não avaliamos é a influência da complexidade espacial da cobertura espacial na paisagem local. Em paisagens contemporâneas em que todo o habitat remanescente está empiricamente aglomerado podemos não observar diferença entre as paisagens, principalmente entre os tipos de paisagens hipotéticas contemporâneos e sem fragmentação. Nesse contexto, eu havia desenvolvido uma métrica de fragmentação per se baseado no contraste da taxa U no simulado entre paisagens contemporâneas e sem fragmentação per se.[y]  A taxa U pode ser usada para desenvolver uma métrica de conectividade que depende tanto da capacidade de dispersão quanto da particular configuração espacial do habitat. Eu defini que o efeito de fragmentação per se é a diferença entre a taxa U estimada na paisagem contemporânea, menos a estimada na paisagem sem fragmentação per se, o resultado dividido pelo estimado na paisagem idealizada. Apliquei essa lógica para todos os 3 possíveis contrastes par a par das 3 paisagens hipotéticas. Na figura SI 3 há o predito para o contraste entre cada possível par de paisagens hipotéticas, na figura \@ref(fig:GE-Urep-sitio-k-land) há as taxas U estimadas. [z]
...
[a]no geral achei uma boa descrição sucinta do que se vê nas figuras. Acho que duas coisas podem melhorar: 
1 - A ordem com que os padrões são descritos. Há algumas descrições de padrões (amarelo) que me parecem meio desconexas no texto. Sei que é difícil juntar tantos padrões num texto consistente, mas senti falta de algumas expressões para guira o leitor.
2 - como são muitos padrões. sugiro 1-2 sentenças resumindo o que se pode concluir de tuo o que foi descrito. Ou pelo menos as tendências mais importantes.
[b]seriam a paisagem sem perda de habitat e a sem fragmentação? Ou as 3 paisagens? Acho que chamar a paisagem observada de hipotética pode confundir. 

De toda forma, em algum ponto do texto é muito importante definir nomes simples destas paisagens e manter sempre estes. Sugiro: paisagem sem perda de habitat, paisagem sem fragmentação e paisagem observada. E para se referir a todas, chamaria de as 3 paisagens usadas apenas.
[c]superiores a qual outra estimativa?
[d]pouca variação nas estimativas de U?
[e]se daqui para frente vc vai falar dessas maiores diferenças, sugiro separar em outro parágrafo
[f]Sugestão para esta seção: 
...
[g]acho que é só redação, mas não está claro
[h]o que pode aumentar?
[i]isso me pareceu muito vago
[j]acho que sempre não é apropriado. Sugiro: congruência constante em relação a k, queda da congruência com aumento de k, e aumento da congruência em relação a k. E aí ajudadrai indicar alguns exemplos para cada caso.
[k]variabilidade do que?
[l]usar os termos em português
[m]boas e ruin quanto a que?
[n]melhores no que?
[o]diversidade de que?
[p]acho que isso já é discussão
[q]Explicação: Além disso, a riqueza de espécies arbóreas na Floresta Atlântica muda em função da topografia, latitude e distribuição de tipos de solo (REF). O esforço amostral também deve depender da disponibilidade operacional do estudo, por exemplo, acesso dos locais candidatos em relação aos centros de pesquisa, e do conhecimento prévio dos locais candidatos (REF). A topografia também deve influenciar as variáveis preditoras, proporção de cobertura vegetal (p) e classe de sucessão, pois a capacidade de mecanização da paisagem pode aumentar a chance da paisagem ter baixa proporção de cobertura vegetal e do local amostrado ser em uma área com pouco tempo de regeneração desde o último grande distúrbio (REF).
[r]isso me parece discussão
[s]qual classificação visual? Veja minha sugestão geral a esta seção.
[t]pq chamar de preditoras, se não há um modelo ajustado?
[u]vago. Se vc vê padrão espacila nesta variáveis, pode usar os resultados para descrevê-los
[v]o único padrão é este? Se sim, batas manter isso. Mas se vc vê outras tendências no mapa, importante descrevê-las de maneira tão clara quanto esta
[w]Massa! Te sugiro explicar esta classificação no início desta seção, ver comentários gerai, no título da seção.
[x]por que? descreva algum resultado ou padrão na figura que ajude a sustentar isto
[y]não entendi como estimar fragmentação per se da paisagem observada resolve o problema explicado nas sentenças anteriores.
[z]Não entendi totalmente o propósito desta parte. Suspeito que pode ficar melhor como itemd e discussão
