...
```
```{r ggplot2code GE nCong,eval=FALSE,include=FALSE,fig.cap=cap}
...
# p
#
# df_plot %>% filter(land_hyp=="cont",k==0.05) %>% pull(forest_succession) %>% table()
laymat <- cbind(rep(1,10),
                c(NA,NA,rep(2,6),NA,NA),
                c(NA,rep(3,6),NA,NA,NA))
f_plot <- \(listplot,laymat=laymat,legend_position="top",textsize=10){
  # legenda comum
  listplot[[1]] <- listplot[[1]] + 
    guides(color=guide_legend(title.theme = element_text(size=textsize*1.5),
                              label.theme = element_text(size=textsize*1.5)))
  plot_table <- ggplot_gtable(ggplot_build(listplot[[1]])) # get tabular interpretation of plot 
  ...
  # títulos associados
  l_title <- llply(listplot,\(li){
    li <- li + theme(plot.title = element_text(size=textsize*1.2))
  ...
  arrenged_plot <- arrangeGrob(grobs=listplot,layout_matrix = laymat) %>% as_ggplot()
  arrenged_plot <- arrangeGrob(arrangeGrob(grobs = l_title,nrow = 1),
                               arrenged_plot,
                               heights = c(0.5, 10)) %>% as_ggplot()
  
  if(legend_position=="bottom"){
    p_return <- grid.arrange(common_legend,arrenged_plot,ncol=1,heights = c(10, 1))
  }else if(legend_position=="top"){
    p_return <- grid.arrange(common_legend,arrenged_plot,ncol=1,heights = c(1, 10))
  }
  return(p_return)
}
```
