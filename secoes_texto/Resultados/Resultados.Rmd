...
# 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
# pacotes
library(trackdown)
# both overwrite: 
...
f_plot <- \(listplot,laymat){
  ...
  # adequação de cada elementeo da lista
  listplot <- llply(listplot,\(li){
    ...
    # remoção de legenda
    li + theme(legend.position = "none",plot.title = element_blank()) # li <- 
    # # arranjo dos gráficos
    # arrangeGrob(p_title,li,ncol=1,heights=c(1,10)) %>% as_ggplot()
  })
  arrenged_plot <- arrangeGrob(grobs=listplot,layout_matrix = laymat)
  if(legend_position=="bottom"){
    p_return <- grid.arrange(common_legend,arrenged_plot,ncol=1,heights = c(10, 1))
  }else if(legend_position=="top"){
    p_return <- grid.arrange(arrenged_plot,common_legend,ncol=1,heights = c(10, 1))
  }
  p_return <- 
  return(p_return)
}
