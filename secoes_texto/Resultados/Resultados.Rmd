...
No modelo neutro espacialmente explícito, quando consideramos a diversidade total da paisagem após um indivíduo ser sorteado para morrer (resultando em extinção ou variação na abundância relativa), a riqueza da paisagem aumenta quando uma nova espécie coloniza uma unidade de habitat, evento com probabilidade U. A riqueza da paisagem se mantem nos 1-U casos, em que a colonização do habitat disponível é feito por um propágulo de uma espécies presente na paisagem.
...
names(l_p) <- v_cols
#
v_ind <- str_which(names(l_p),"ratio")
l_p[v_ind+3] <- llply(names(l_p)[v_ind],\(i){
  l_p[[i]] + 
    scale_y_continuous(trans = "log10") +
    labs(
      y=case_when(i == "area_ratio" ~ "log10(aglomerada / prístina)",
                  i == "frag.perse_ratio" ~ "log10(contemp / aglomerada)",
                  i == "frag.total_ratio" ~ "log10(aglomerada / prístina)")
      )})
names(l_p)[v_ind+3] <- gsub("ratio","logratio",names(l_p)[v_ind])
######
