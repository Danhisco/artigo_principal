...
arrenged_plot <- arrangeGrob(grobs=l_p,layout_matrix = laymat) %>% as_ggplot()
arrenged_plot <- arrangeGrob(arrangeGrob(grobs = l_title,nrow = 1),
                             arrenged_plot,
                             heights = c(0.5, 10)) %>% as_ggplot()
if(legend_position=="bottom"){
  p <- arrangeGrob(common_legend,arrenged_plot,ncol=1,heights = c(10, 1)) %>% as_ggplot()
}else if(legend_position=="top"){
  p <- arrangeGrob(common_legend,arrenged_plot,ncol=1,heights = c(1, 10)) %>% as_ggplot()
}
p2 <- df_plot %>% filter(k==0.05) %>% distinct() %>% 
