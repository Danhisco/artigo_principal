...
l_df <- list()
l_df[[1]] <- data.frame(`predição` = c("taxa U","SAD"),
                        `descrição` = c("Probabilidade per nascimento 
                                        do colonizador do habitat disponível ser uma nova espécie na paisagem",
                                        "Distribuição de Abundância de Espécies na área amostrada"),
                        `comparações` = c("em um contraste, 
                                          a diferença na taxa U média 
                                          dividido pela tava U média estimada em paisagens sem perda de habitat",
                        ...
                        obs = c("estimado para obter a riqueza observado",
                                "aplicado entre SADs preditas e entre preditas e empírica"))
l_df[[2]] <- data.frame(contraste = c("área per se","fragmentação per se","contemporaneidade"),
...
l_df[[3]] <- data.frame(nome = c("k","p","areaU","fragU","contempU","areaSAD","fragSAD","contempSAD"),
  `descrição` = c("grau de limitação de dispersão per capita",
                  ...
                  "contraste de área per se na congruência da SAD predita",
                  "contraste de frag. per se na congruência da SAD predita",
                  "contraste de contemp.  na congruência da SAD predita"))
v_footnote <- c(
  NA,
...
names(l_df) <- c("predições",
                 "contrastes",
                 "preditoras")
v_title <- c("tipos de predições simuladas",
...
O GAMM que descreve o contraste de fragmentação per se em função do contraste de área per se não apresenta um boa predição dos valores observados (Figura \@ref{frag_area_taxaU} e Apêndice 2, avaliação GAMM 2). O intervalo de predição sugere que o contraste de fragmentação per se aumenta com o contraste de área per se, porém, em valores diminutos de área per se há diversos valores elevados de fragmentação per se (Figura \@ref{frag_area_taxaU}). 
...
```{r frag ~ area Taxa U, fig.cap="\\label{frag_area_taxaU}frag ~ area Taxa U",eval=FALSE,include=FALSE}
load("./dados/Rdata/frag_area_taxaU.Rdata")
df |> 
...
<!-- __Figura \@ref{frag_area_taxaU}__ Intervalo de Predição estimado pelos GAMM.  -->

## congruência da SAD predita 

Tabela \@ref{ts_congCont} (tabela de seleção dos gamm usados para descrever congCont)

```{r ts congCont,cache=TRUE,tab.cap="\\label{ts_congCont}ts congCont"}
...
```{r ip congCont,fig.cap="\\label{ip_congCont}ip congCont"}
load("./dados/Rdata/ip_congCont.Rdata")
grid.arrange(grobs=l_p,
             layout_matrix=rbind(c(1,2),
                                 c(3,2)))
```

__Figura \@ref{ip_congCont}__ Congruência da SAD predita entre contrastes: observada e estimada quando se pressupõe ausência de variabilidade entre sítios. 

# SAD observada e SAD predita entre paisagens hipotéticas

## Diferença na Riqueza Estimada

__Tabela \@ref{ts_diffs}__ Seleção dos LMERs usados para descrever o erro na Riqueza Estimada

```{r ts diffS,tab.cap="\\label{ts_diffs}ts diffS"}
load("./dados/Rdata/ts_diffs_E_ip_diffS")
df_seldiffs |> 
  gt()
```
...
```{r diffS md + lik ip,fig.cap="\\label{ip_diffS}ip diffS"}
p
```

__Figura \@ref{ip_diffS}__ Intervalo de predição diffS

## Pr(Cong) ~  landscape hypothesis

__tabela \@ref{ts_prcong}__ GAMMs usados para descreever a probabilidade de congruência com o observado. Em todos os modelos há um spline fixo e aleatório para a variável k (grau de limitação de dispersão )
...
#### figuras para descrever Probabilidade de Congruência

```{r}
f_plot <- \(df_md,newpred_path="./dados/csv/df_ad4.csv",mdRdata_path="./dados/Rdata/md_prcong.Rdata"){
  # model
  md_name <- load(mdRdata_path,verbose=TRUE)
  md <- eval(as.name(md_name))
  rm(list=md_name)
  # df_newpred sem variabilidade entre sítios: efeito médio em função da preditora
  if(file.exists("./dados/csv/df_ad4.csv")){
    df_newpred <- read_csv(file="./dados/csv/df_ad4.csv")  
  }else{
    df_newpred <- f_calcPI(gamm=md,
                           ad = "ad4",
                           newdata_path="obs",
                           link_scale = TRUE)
    write_csv(df_newpred,file="./dados/csv/df_ad4.csv")
  }
  # df_predtoobs
  df_md$predtoobs <- predict.gam(md)
  df_newpred <- df_newpred %>% 
    arrange(k_z,areaU_z,fragU_z) %>% 
    cbind(.,df_md %>% 
          arrange(k_z,areaU_z,fragU_z) %>% 
          select(SiteCode,nCong,predtoobs)) %>% 
    relocate(SiteCode) %>% 
    mutate(logitoObs = case_when(nCong==100~99.9,
                                 nCong==0~0.1,
                                 TRUE~nCong),
           logitoObs = log((logitoObs/(100-logitoObs))))
  # l_p
  df_newpred %>% 
    pivot_longer(starts_with("Q_0.")) %>% 
    filter(name=="Q_0.5") %>% 
    ggplot(aes(x=k_z,y=logitoObs)) +
    geom_point(alpha=0.3) +
    geom_point(aes(y=value),color="red",alpha=0.3) +
    facet_wrap(~land_hyp)
}
