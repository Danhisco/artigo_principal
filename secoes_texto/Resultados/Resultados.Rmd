...
## df_ad: resultados completos + preditoras comuns, falta as métricas de efeito da paisagem
df_ad <- df_resultados  %>% 
  inner_join(x=df_p,y=.) %>% 
  arrange(p) |> 
  inner_join(distinct(select(df_sim,SiteCode,Ntotal:S_obs)),"SiteCode") |> 
  mutate(diffS = (Smed - S_obs)/S_obs) %>% 
  inner_join(df_contrastes,by=c("SiteCode","k"))
...
p + theme(plot.title = element_text(size = 12)) + theme_bw()
```
...
p <- df_plot |>
  ...
  labs(x="k",
       ...
       title="A escala de 4x4 km2 é adequada para todos os graus de dispersão") +
  theme_bw() + 
...
A partir dessa seção iremos analisar os dados das simulações feitas nos 3 tipos de paisagens hipotéticas: a paisagem contemporâneas, com a configuração espacial do habitat remanescente observada no mesmo ano que a melhor aproximação do ano da amostragem da parcela; a paisagem sem fragmentação per se, onde todo o habitat remanescente na paisagem contemporânea é aglomerado ao redor  da parcela em forma de quadrado; e a paisagem prístina, sem perda de habitat. Ao ordenar as estimativas da taxa U pela proporção de cobertura florestal na paisagem ao redor do sítio de amostragem (fig. \@ref(fig:GE-Urep-sitio-k-land)), a maior cobertura florestal no qual se observa diferença entre a taxa U estimada nas diferentes paisagens é de 0.92 (sítio de amostragem RSrioz1). Nessa paisagem, o estimado na paisagem contemporânea foi superior em graus de limitação brandos (fig. \@ref(fig:GE-Urep-sitio-k-land)). Abaixo dessa proporção de cobertura remanescente, a maior variação entre graus de limitação de dispersão ocorre nos graus de limitação de dispersão mais brandos simulados, seguido pelos graus mais severos. Nos graus de limitação de dispersão pouco severos, onde há o máximo global nas paisagens infinitas e sem perda de habitat (olhar seção ‘Efeito de Escalar’), ocorre a menor variabilidade entre tipos de paisagens hipotéticas (fig. \@ref(fig:GE-Urep-sitio-k-land)). Em geral, a paisagem hipotética contemporânea apresenta estimativas superiores da taxa U em comparação com as outras duas paisagens hipotéticas (fig. \@ref(fig:GE-Urep-sitio-k-land)). Em algumas paisagens com proporção de cobertura remanescente abaixo dos 0.50 o estimado na paisagem aglomerada é superior ao estimado na paisagem prístina, em poucas é superior ao estimado na paisagem contemporânea (fig. \@ref(fig:GE-Urep-sitio-k-land), na 2a coluna e 4a linha e na 10a coluna e 1a linha ). Sobre a estimativa da taxa U, em resumo: i) quanto maior a perda de habitat e mais branda a limitação de dispersão, maior a potencial diferença entre as 3 paisagens hipotéticas; ii) em geral a estimativa em paisagens contemporâneas é superior ao estimado nos outros 2 tipos de paisagens hipotéticas; iii) em paisagens com pouca cobertura florestal o estimado na paisagem aglomerada pode ser superior ao estimado na paisagem prístina, principalmente em graus brandos de limitação de dispersão. 
...
<!-- __\@ref(fig:GE-Urep-sitio-k-land)__   -->

\newpage

### Métricas de efeito da paisagem ao redor

As métricas de efeito da paisagem foram obtidas pela comparação da estimativa da taxa U entre paisagens hipotéticas para um mesmo sítio de amostragem e grau de limitação de dispersão. As métricas baseadas na taxa U podem ter interpretações no nível da dinâmica ecológica simulada pelo modelo neutro. No modelo neutro espacialmente explícito, quando consideramos a diversidade total da paisagem após um indivíduo ser sorteado para morrer (resultando em extinção na paisagem ou variação na abundância relativa), a riqueza da paisagem aumenta quando uma nova espécie na paisagem coloniza uma unidade de habitat, evento com probabilidade U. A riqueza da paisagem se mantem nos 1-U casos, em que a colonização do habitat disponível é feito por um propágulo de uma espécies presente na paisagem.
Quando consideramos a riqueza local na parcela amostrada, a interpretação da probabilidade U é a mesma: como o nascimento de uma nova espécie na paisagem que resulta tanto no aumento da riqueza local e total. A interpretação de 1-U muda na parcela, pois, o nascimento pode ser de um indivíduo de uma espécie presente na parcela ou de uma nova espécie na parcela. Assim, os 1-U eventos na parcela que aumentam a riqueza local são certamente resultado da imigração de um propágulo de fora da parcela. Os 1-U eventos na parcela que mudam a abundância relativa de espécies locais pode ocorrer por propágulos locais ou imigrantes. 
Portanto, a conectividade entre parcela e paisagem ao redor simulada é proporcional à 1-U, uma vez que a taxa U é ajustada para que a riqueza local simulada seja igual à riqueza local observada. Como a única diferença entre simulações de um mesmo sítio de amostragem e grau de limitação de dispersão é a paisagem hipotética, podemos atribuir diferenças na taxa U estimada entre paisagens hipotética como efeito da paisagem ao redor na probabilidade 1-U dos nascimentos na parcela.

Como as estimativas podem ser interpretadas como probabilidades por evento de nascimento algumas possíveis formas de comparar essas estimativas par a par são pela diferença, razão, log da razão e log odds ratio (\@ref(GE_contrastes)). O log da razão e o log odds ratio apresentam alta similariedade nas formas pois os valores de U são muito pequenos, de modo que 1-U é próximo de 1. O padrão das comparações entre estimativas reflete o observado nas taxas U (\@ref(fig:GE-Urep-sitio-k-land)). A taxa U estimada na paisagem contemporânea em geral é superior ao estimados nas outras paisagens hipotéticas; em algumas casos, a paisagem aglomerada apresentou taxa U superior, indicado pela comparação entre contemporânea e aglomerada abaixo do observado em paisagens com pouca perda de cobertura (p>=0.93).

```{r caption contrastes1 GE}
cap <- "Contraste entre pares de paisagens hipotéticas (indicadas no topo dos paineis) e possíveis formas de comparação entre as paisagens hipotéticas (eixos y) por grau de limitação de dispersão (eixo x). Os pontos representam cada comparação e aqueles de paisagem com cobertura florestal igual ou superior à 0.93, situação em que não se observa diferença entre paisagens hipotéticas, estão em verde."
```
```{r criacao da fig1 dos contrastes,eval=FALSE,include=FALSE,echo=FALSE}
#### caption
######
v_cols <- select(df_contrastes,matches("dif|ratio|logit")) %>% names()
l_p2 <- df_contrastes %>%
  ...
  mutate(classe = case_when(grepl("area",efeitos) ~ "aglomerada, prístina",
         ...
         tipo = case_when(grepl("dif",efeitos) ~ "diferença",
                          ...
                          grepl("logit",efeitos) ~ "logOR"),
         tipo = factor(tipo,levels=c("diferença","razão","log10(razão)","logOR")),
         color=ifelse(p>=0.93,"p>=0.93","p<0.93")) %>%
  ggplot(aes(x=k, y=valores,group=SiteCode,col=color)) +
  geom_line(alpha=0.7) +
  geom_point(alpha=0.7) +
  scale_color_manual(values=c("p>=0.93"="darkgreen",
                              "p<0.93"="darkgray")) +
  scale_x_reverse() +
  # coord_flip() +
  labs(y="",
       x="k (aumenta a limitação de dispersão)") +
  theme(legend.title = element_blank(),
        legend.position = "top",
        strip.placement.y = "outside",
        strip.background = element_blank(),
        strip.text = element_text(face = "bold")) +
  facet_grid(tipo ~ classe, scales="free", switch = "y")
# l_plot <- list()
# l_plot$plot <- l_p2
# l_plot$caption <- cap
save(l_p2,file="./secoes_texto/Resultados/figuras/l_plot_contrastes1.Rdata")
```
```{r GE_contrastes,eval=TRUE,fig.height=figHeight,fig.width=figWidth,fig.cap=cap,fig.pos="H"}
load("./secoes_texto/Resultados/figuras/l_plot_contrastes1.Rdata")
l_p2
```
...
### Paisagens com alta proporção de cobertura florestal

```{r caption fig1 nCong GE}
cap <- "Número de SADs simuladas congruentes com a SAD empírica (eixo y) por sítio de amostragem (título dos paineis) e grau de limitação de dispersão (eixo x). Os pontos estão coloridos pelo tipo de paisagem hipotética e a forma dos pontos se o U foi ajustado livremente ou se foi usado o U estimado na paisagem prístina."
```
```{r GE-nCong-p093oumaior,fig.cap=cap}
#
df_ad <- df_resultados %>% 
  inner_join(df_p) %>% 
  filter(p>=0.93)
# 
df_ad %>% 
  ggplot(aes(x=k,y=nCongKS,color=land_type,shape=Uestimad)) +
  geom_line((aes(linetype=Uestimad)),alpha=0.75) +
  geom_point(size=1,alpha=0.75) +
  scale_x_reverse() +
  labs(x="k(aumenta a limitação de dispersão)",
       y="número de SADs simuladas congruentes com a SAD empírica") +
  facet_wrap(~SiteCode)
```

### Paisagens no gradiente de cobertura florestal

```{r caption fig1 nCong GE}
cap <- "Número de SADs simuladas congruentes com a SAD empírica (eixo y) por sítio de amostragem (título dos paineis) e grau de limitação de dispersão (eixo x). Os pontos estão coloridos pelo tipo de paisagem hipotética e a forma dos pontos se o U foi ajustado livremente ou se foi usado o U estimado na paisagem prístina."
```
```{r GE-nCong-p092oumenor,fig.cap=cap}
#
df_ad <- df_resultados %>% 
  inner_join(df_p) %>% 
  filter(p<0.93) %>% 
  arrange(p) %>% 
  mutate(label = paste0(SiteCode, ", p = ",round(p,2)))
df_ad$label <- factor(df_ad$label,levels=unique(df_ad$label))
# 
df_ad %>% 
  ggplot(aes(x=k,y=nCongKS,color=land_type,shape=Uestimad)) +
  geom_line((aes(linetype=Uestimad)),alpha=0.75) +
...
### Fragmentação per se é relevante?

```{r}
df_md <- inner_join(df_contrastes %>%select(SiteCode,k,matches("frag.perse|area")),
                    df_ad %>% filter(land_type %in% c("contemp","non_frag")),
                    by=c("SiteCode","k")) %>% 
  select(SiteCode:nCongKS,Ntotal:diffS) %>% 
  rename(nCong=nCongKS) %>% 
  relocate(nCong,.after = "k") %>% 
  relocate(c(p,land_type,Uestimad),.before="k")
```
