...
Quando há pouca perda de habitat a estimativa da taxa U não difere entre paisagens hipotéticas (fig. \@ref(fig:GE-Urep-sitio-k-land)). Com a redução da cobertura vegetal a estimativa da taxa U pode ser distinta entre paisagens hipotéticas. Em graus de limitação de dispersão mais relaxados pode existir o aumento da taxa U estimada nas paisagem contemporâneas ou sem fragmentação (fig. \@ref(fig:GE-Urep-sitio-k-land)). Em geral a taxa U estimada nas paisagens contemporâneas é a maior das três paisagens hipotéticas (fig. \@ref(fig:GE-Urep-sitio-k-land)). Em alguns casos a paisagem sem fragmentação per se pode apresentar estimativas superiores (fig. \@ref(fig:GE-Urep-sitio-k-land)). Nos graus onde há pouca variação no efeito de escalar (k entre 0.80 e 0.55) também há pouca variação entre paisagens hipotéticas (fig. \@ref(fig:GE-Urep-sitio-k-land)). As maiores diferenças entre paisagens hipotéticas ocorre em graus de limitação de dispersão mais brandos (fig. \@ref(fig:GE-Urep-sitio-k-land)). Em paisagens com baixa proporção de cobertura florestal as estimativas em graus severos de limitação de dispersão também podem ser distintas entre paisagens hipotéticas (fig. \@ref(fig:GE-Urep-sitio-k-land)). Em alguns sítios a taxa U estimada em paisagem sem isolamento estrutural pode ser superior ao estimado em paisagens contemporâneas (fig. \@ref(fig:GE-Urep-sitio-k-land)). Em geral a diferença na estimativa entre paisagens idealizadas e sem fragmentação é pequena e a diferença na estimativa entre paisagens idealizadas e contemporâneas pode ser maior (fig. \@ref(fig:GE-Urep-sitio-k-land)).
...
Na figura \@ref(fig:GE-nCong) há os gráficos de congruência da SAD simulada e observada (número de SADs simuladas congruentes com a SAD observada) em cada sítio de amostragem. No eixo x há o grau de limitação de dispersão per capita (k, proporção de propágulos que permanece até a vizinhança imediata da progenitora). O fundo dos painéis está colorido pela proporção de cobertura vegetal (p). Os painéis estão agrupados pela classe de estado sucessional da base de dados TreeCo. No canto inferior direito a distribuição de sítios em função de p e da classe de sucessão. 
Em paisagens com cobertura vegetal abaixo de 0.92 é possível observar, em alguns graus de limitação de dispersão, diferença no predito entre paisagens hipotéticas, apesar da similaridade. A diferença entre paisagens hipotéticas pode aumentar com a redução da cobertura vegetal.  O predito em paisagens idealizadas e sem fragmentação apresenta menor divergência.  Em geral, o predito em paisagens contemporâneas pode apresentar congruência melhor ou pior, com maior diferença das outras duas paisagens hipotéticas, em graus de limitação de dispersão mais brandos. Existe muita diversidade entre sítios, mas podemos classificar os padrões de congruência com o observado em tipos: sempre bom ou ruim; ruim ou bom em graus de limitação de dispersão brandos; ruim ou bom em graus de limitação de dispersão intermediários. Ou seja, pode apresentar variabilidade em todo o gradiente de limitação de dispersão simulada, com maior variabilidade em graus brandos de limitação de dispersão, seguido de graus severos, e menor variabilidade em graus de limitação intermediários. A classe sucessional do sítio de amostragem parece aumentar a variabilidade no padrão de congruência: em sítios primary e secondary, paisagens contemporâneas parecem ser tão boas quanto ruins; em sítios secondary, paisagens idealizadas tendem a ser melhores



Há muita diversidade de padrões entre sítios de amostragem na figura \@ref(fig:GE-nCong). Em alguns há pouca variação na congruência da SAD simulada e predita, nesses casos, em geral  com boa congruência para todos os graus de limitação de dispersão e tipos de paisagens (figura \@ref(fig:GE-nCong)). Em outros há variação na congruência com a variação no grau de limitação de dispersão, nesses casos pode existir variabilidade entre tipos de paisagens hipotéticas, em que o tipo de paisagem hipotética com melhor congruência pode variar entre sítios (figura \@ref(fig:GE-nCong)). Ao ordenar os gráficos dos sítios por percentagem de cobertura vegetal na paisagem de 4x4km (p) e classe de sucessão ecológica é possível observar que os sítios com pouca perda de habitat não apresentam diferença entre os tipos de paisagens hipotéticas (figura \@ref(fig:GE-nCong)). A partir de um valor de p é possível observar diferenças entre tipos de paisagens (figura \@ref(fig:GE-nCong)). 


O simulado nas paisagens idealizadas e sem isolamento estrutural em geral apresentam diferença menor e o simulado na paisagem contemporânea pode apresentar maior diferença em alguns graus de limitação de dispersão (figura \@ref(fig:GE-nCong)). 
...
v_size = 3
# v_w <- 860
png("./figuras/fig6_final.png",width = 860 * v_size, height = 792 * v_size,res = 72 * v_size)
l_plot[[1]]
...
```
```{r GE-nCong,eval=TRUE,fig.height=figHeight-4,fig.width=figWidth,fig.cap=cap,fig.pos="H",cache=FALSE}
