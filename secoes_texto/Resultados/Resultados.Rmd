...
As métricas de efeito da paisagem foram obtidas pela comparação entre a estimativa da taxa U entre paisagens hipotéticas para um mesmo sítio de amostragem e grau de limitação de dispersão. As métricas baseadas na taxa U podem ter interpretações no nível da dinâmica ecológica simulada pelo modelo neutro. No modelo neutro espacialmente explícito, quando consideramos a diversidade total da paisagem após um indivíduo ser sorteado para morrer (resultando em extinção na paisagem ou variação na abundância relativa), a riqueza da paisagem aumenta quando uma nova espécie coloniza uma unidade de habitat, evento com probabilidade U. A riqueza da paisagem se mantem nos 1-U casos, em que a colonização do habitat disponível é feito por um propágulo de uma espécies presente na paisagem.
...
l_p2 <- df_contrastes %>%
  ...
  mutate(classe = case_when(grepl("area",efeitos) ~ "aglomerada, prístina",
         ...
         tipo = case_when(grepl("dif",efeitos) ~ "diferença",
                          ...
                          grepl("logit",efeitos) ~ "logit"),
         tipo = factor(tipo,levels=c("diferença","razão","log10(razão)","logit")),
...
l_plot$plot <- l_p2
l_plot$caption <- cap
save(l_plot,file="./secoes_texto/Resultados/figuras/l_plot_contrastes1.Rdata")
```
...
### Paisagens com pouca perda de cobertura florestal
