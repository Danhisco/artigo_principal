...
#   select(-c(N:log.S)) %>% 
#   pivot_longer(cols = all_of(matches("dif|ratio|logratio")),
#                names_to = "efeitos",
#                values_to="valores") %>%
#   mutate(classe = case_when(grepl("area",efeitos) ~ "aglomerada, prístina",
#                             grepl("perse",efeitos) ~ "contemp, aglomerado",
#                             grepl("total",efeitos) ~ "contemp, prístina"),
#          tipo = case_when(grepl("dif",efeitos) ~ "diferença",
#                           grepl("_ratio",efeitos) ~ "razão",
#                           grepl("logratio",efeitos) ~ "log10(razão)"),
#          tipo = factor(tipo,levels=c("diferença","razão","log10(razão)")))
...
# write_csv(df_plot,"dados/csv/df_plot_geral.csv")
# # 
# registerDoMC(3)
# df_resultados_completos <- read.csv("./dados/csv/resultados_MN/MNEE/df_repMNEE2.csv") %>% 
#   ddply(.,c("SiteCode","k","land_type"),f_summarise_SAD_MNEE,.parallel = TRUE) %>% 
#   mutate(Uestimado="U prístino") %>% 
#   rbind.fill(df_resultados %>% 
#                mutate(Uestimado="U livre") %>% 
#                select(SiteCode,k:Smax,Uestimado)) %>% 
#   arrange(SiteCode,k) %>% 
#   relocate(Uestimado,.after="land_type")
```


# taxa U

## média geral e por sítio a partir de U rep

__modelos ajustados__
```{r U media a partir da U rep}
df_md <- df_Urep %>% 
  pivot_longer(cols = -c(SiteCode:d),names_to = "rep",values_to = "U") %>% 
  mutate(k = factor(round(k,digits = 2)))
l_f <- list()
l_f[[1]] <- "U ~ land_type * k + (k|SiteCode)"
l_f[[2]] <- "U ~ k + (k|SiteCode)"
names(l_f) <- c("U ~ land type * k", "U ~ k")
registerDoMC(2)
l_md <- llply(l_f, \(li) lmer(as.formula(li),data=df_md),.parallel = TRUE)
AICctab(l_md,weights=TRUE)
saveRDS(l_md,file="./apendices/analise_dados/Rdata/l_md_taxaUrep.rds")
```

__comparação de modelos e diagnosticos__
```{r}
# tabela de seleção
l_md <- readRDS("./apendices/analise_dados/Rdata/l_md_taxaUrep.rds")
df_tab <- l_md %>% f_tselR2()
saveRDS(df_tab,file="./secoes_texto/Resultados/dados/df_tselR2_Urep.rds")
# gráficos diagnósticos
f_resDharma(md = l_md[[df_tab$formula[1]]],
            png_fullpath = "./secoes_texto/Resultados/figuras/resDharma_Urep_maisplaus.png",
            width = 800,height = 600,units = "px", res = 100)
resDharma <- simulateResiduals(md,n=1000)
plot(resDharma)
```
...
#### dados ####
l_filter <- df_md %>% 
  select(SiteCode,forest_succession,forest_type) %>% 
  distinct() %>% 
  pivot_longer(-SiteCode,names_to="preditora",values_to="categoria") %>% 
  group_by(preditora) %>% 
  count(categoria) %>% 
  filter(n>=5) %>% 
  as.data.frame %>%
  dlply(.,"preditora",\(i) i$categoria)
# df especifico
v_cols <- str_detect(names(df_resultados), "^S(?!.*de$)")
df_md <- df_resultados[,!v_cols] %>% 
  filter(land_type!="pristin") %>% 
  rename(nCong = nCongKS) %>% 
  inner_join(df_p) %>% 
  inner_join(df_dados_disponiveis %>% 
               select(SiteCode,forest_type,forest_succession)) %>% 
  inner_join(df_contrastes %>% select(SiteCode:k,starts_with("frag.perse"))) %>% 
  filter(forest_succession %in% l_filter[["forest_succession"]],
         forest_type %in% l_filter[["forest_type"]])
# formulas
v_f <- "cbind(nCong,100-nCong) ~ land_type * Uestimad * metrica + control_var + (land_type * Uestimad * metrica|SiteCode)"
v_cols <- names(select(df_md,starts_with("forest")))
v_cols[length(v_cols)+1] <- paste(v_cols,collapse = " + ")
l_f <-  lapply(v_cols,\(li){
  v_f <- sub("control_var",li,v_f)
  v_cols <- names(select(df_md,starts_with("frag.perse")))
  lapply(v_cols,\(x){
    gsub("metrica",x,v_f)
  })
}) %>% do.call(c,.)
f_glmm <- \(vf,df_data=df_md){
  glmer(as.formula(vf),
        family='binomial',
        data=df_data,
...
registerDoMC(3)
l_md <- llply(l_f,f_glmm,.parallel = TRUE)
save(l_md,file="./secoes_texto/Resultados/dados/l_md_ncong.Rdata")
...
Não é possível utilizar as variáveis categoricas tipo florestal e grau de perturbação florestal com interação, não há exemplares sucificentes. Para usar essas variáveis será necessário remover 2 observações que apresentam 1 classe única de tipo floresal e 2 observações que na classificação de perturbação são capoeiras.
...
df_md %>% 
  ...
  facet_wrap(~preditora,scales="free") +
  theme(axis.text.x = element_text(angle = 90))
  # count(forest_type,forest_succession) %>%
  # ggplot(aes(x=forest_type,y=forest_succession,fill=n)) + 
  # geom_tile()
```
...
```{r classificação dos sítios por tipo de congruência}
df_class <- adply(df_plot$SiteCode %>% unique,1,\(Site){
  df_return <- data.frame(SiteCode = Site,land_diff = "NA",Cong = "NA")
  p <- df_plot %>% 
    filter(SiteCode == Site) %>% 
    ggplot(aes(x=k,y=nCong,color=land_hyp)) +
      geom_point(alpha=0.5) +
      geom_line(alpha=0.5) +
      scale_x_reverse() +
      scale_y_continuous(limits = c(0,100)) +
      theme_bw() +
      labs(color="landscape hyp.",title=Site) +
  ...
  x11()
  print(p)
  df_return <- edit(name = df_return)
  dev.off()
  return(df_return)
},.id=NULL)
Site <- df_class %>% filter(land_diff=="contemp") %>% pull(SiteCode)
df_class[df_class$land_diff=="contemp","land_diff"] <- "contemp pior"
df_class[df_class$land_diff=="contemp pio","land_diff"] <- "contemp pior"
df_class$land_diff %>% table()

df_class[df_class$Cong=="boa excetoe em graus brandos","Cong"] <- "boa exceto em graus brandos"
df_class$Cong %>% table()
df_class <- df_class[,-4]
write_csv(df_class,file="./dados/csv/df_class.csv")
```
```{r união das classes}
...
df_class <- df_class %>% mutate(
  classi_land = case_when(land_diff %in% c("similar","igual") ~ "similares",
                          land_diff %in% v_landdiff[str_which(v_landdiff,"contemp")] ~ "contemp. diferente", 
                          land_diff %in% v_landdiff[str_which(v_landdiff,"ideal")] ~ "ideal. diferente",
                          land_diff == "sem frag pior" ~ "sem frag. diferente"),
  classi_cong = case_when(Cong %in% c("ruim exceto em graus brandos","melhor em graus brandos") ~ "boa em graus brandos",
                          Cong == "ruim em intermediário e boa em graus brandos" ~ "ruim em graus intermediários",
                          TRUE ~ Cong))
df_class$classi_cong %>% table()
```
...
ggdag_adjustment_set(dag_exp.frag,shadow = TRUE,text = FALSE, use_labels = "label") +
  theme_dag_gray() +
  scale_color_manual(values=c("red4","green4"))
################
dag_exp.frag <- dagify(logOR ~ Uefeito + spatial_conf + k + biogeoreg + year_data,
                        Uefeito ~ k + spatial_conf,
                        spatial_conf ~ coord + year_data,
                        biogeoreg ~ coord + year_data,
                        # p ~ obs_scale + spatial_conf,
                        obs_scale ~ k,
                        labels = c(
                          "logOR" = "Cong. Relat.\nSADobs (logOR)",
                          "Uefeito" = "Efeito em S",
                          "spatial_conf" = "conf. espacial",
                          "k"="k",
                          "biogeoreg" = "reg. biogeo.",
                          "year_data" = "ano do invent.",
                          "coord" = "coord. geo",
                          "obs_scale"="escala obs."
                          ),
                       outcome = "logOR",
                       latent="spatial_conf",
                       exposure = "Uefeito",
...
ggdag_adjustment_set(dag_exp.frag, shadow = TRUE) + 
  ggtitle(label = "Exposure = Uefeito") +
  theme(
    plot.title = element_text(size = 14, face = "bold"),  # Customize plot title
  ...
  theme_dag() +
  # geom_dag_node(color = "blue", size = 10, alpha = 0.7) +  # Customize node color
  # geom_dag_edges(color = "red", size = 1) +                # Customize edge color
...
v_sites <- df_plot %>% arrange(p) %>% pull(SiteCode) %>% unique
df_plot %>% 
  mutate(SiteCode = factor(SiteCode,
                           levels = v_sites)) %>% 
  filter(p>=0.975) %>% 
  ...
  scale_color_manual("Contraste:",
                     values=c("frag.total"="darkred",
                              "frag.perse"="darkblue",
                              "area"="darkgreen")) +
  labs(x="k (aumenta a limitação de dispersão)",y="log( Ui / Uj )") +
  facet_wrap(~SiteCode,ncol=5)
...
df_plot %>% 
  # filter(name==i) %>% 
  mutate(label = case_when(grepl("area",name) ~ "Contraste Área per se: log(U aglomerado / U prístino)",
  ...
  # geom_vline(aes(linetype="amplitude contrastes paisagens com p>=0.975",
  #                xintercept = v_range_RefNulo),color="darkred") +
  # scale_linetype(guide=guide_legend(override.aes = list(color=c("darkred")))) +
...
#   geom_jitter() +
#   geom_boxplot() +
#   geom_hline(yintercept = 1,color="darkred",linewidth=1)
# # l_p[[2]] <- 
# df_plot %>% 
...
#   theme_bw() + 
#   geom_boxplot(aes(fill=name,y=-50,color=name),size=10,alpha=0.1) +
...
#   geom_histogram(bins = 120,aes(x=value),inherit.aes = FALSE) +
...
#   # facet_grid(cut ~ .) +
#   # geom_vline(xintercept = 1,col="darkred",linewidth=1)+
#   
...
```{r GAM contraste taxa U logratio}
...
l_md <- dlply(df_md,"name",f_gam)
saveRDS(l_md,file="dados/Rdata/l_md.contrastes.Rdata")
```
...
```{r intervalo de predição para os contrastes segundo estimado pelo GAMM,fig.height=8,fig.width=12,eval=FALSE}
df_plot <- df_intPred |> # trocar df_intPred pelo novo ou remover o chunk
...
df_md <- inner_join(df_plot,
  ...
  mutate(across(.cols=c(SiteCode,forest_succession,land_hyp),factor))
v_f <- "cbind(nCong,100-nCong) ~ p_z * forest_succession * k_z * land_hyp + s(lat,long) + s(land_hyp,SiteCode,bs='re')"
f_gam <- \(stringf){
  gam(formula = as.formula(stringf),data=df_md,family = binomial)
}
md_latlong <- gam(formula = as.formula(v_f),data=df_md,family = "binomial",method="REML")
```


```{r ncong estrutura aleatória}
l_f <- list()
...
l_md <- list.files(path = "./dados/Rdata",pattern="glmm_re_",full.names = TRUE)[2] %>% 
  f_loadll()
l_md <- l_md[[1]]
l_md <- llply(l_md,f_refit)
df_ts_re <- l_md %>% f_ts_R2()
write_csv(df_ts_re,file="./dados/csv/df_ts_re_nCong.csv")
```

## seleção da estrutura fixa

```{r l_md_nCong}
# formulas usadas para selecionar a estrutura aleatória
l_f <- list()
l_f[[1]] <- "~ p_z * forest_succession * k_z * land_hyp + (land_hyp|SiteCode)"
l_f[[2]] <- "~ p_z * forest_succession * k_z * land_hyp + (1|SiteCode)"
names(l_f) <- laply(l_f,\(s) sub(" \\+ \\(","",trimws(str_extract(s, "\\((.*?)\\)"))))
# tabela de seleção da estrutura aleatória
df_ts_re <- read_csv(file="./dados/csv/df_ts_re_nCong.csv")
# formulas para comparação da estrutura fixa
f0 <- l_f[[df_ts_re$modelo[1]]]
l_f <- list()
l_f[[1]] <- f0 %>% sub("p_z \\* ","",.)
...
l_f <- c(f0,l_f)
l_f <- c(l_f,
         l_f %>% gsub("land_hyp","1",.) %>% gsub(" \\* 1","",.))
names(l_f) <- laply(l_f,\(s) str_extract(s,"(?<=~ )(.*?)(?= \\+)"))
# ajuste das formulas
f_glmm(l_f,df_data=df_plot)
# leitura e refit se necessário
oname <- formals(f_glmm)$Rdata_path
oname <- list.files(path = sub("\\/glmm_","",oname),pattern = "glmm_l_md_nCong",full.names = TRUE)
oname <- load(file=oname)
l_md <- get(oname)
rm(list=oname)
l_md <- llply(l_md,f_refit)
oname <- formals(f_glmm)$Rdata_path
oname <- list.files(path = sub("\\/glmm_","",oname),pattern = "glmm_l_md_nCong",full.names = TRUE)
save(l_md,file=oname)
# tabela de seleção
df_ts <- l_md %>% f_ts_R2()
write_csv(df_ts,file = "./dados/csv/df_ts_nCong.csv")
# modelo mais plausível
md <- l_md[[df_ts$modelo[1]]]$modelo
save(md,file=paste0(formals(f_glmm)$Rdata_path,"md_nCong.Rdata"))
```
...
names(l_f) <- laply(l_f,\(s) str_extract(s,"(?<=~ )(.*?)(?= \\+)"))
l_f2 <- c(f0,l_f)
names(l_f2)[1] <- c("p_z * forest_succession * k_z * land_hyp")
l_f2 <- l_f2 %>% gsub("land_hyp","1",.) %>% gsub(" \\* 1","",.)
names(l_f2) <- laply(l_f2,\(s) str_extract(s,"(?<=~ )(.*?)(?= \\+)"))
l_md2 <- l_f2 %>% f_glmm(.,df_data=df_plot,Rdata_path="./dados/Rdata/glmm_",only_nCong = TRUE,re=TRUE)
l_md2 <- load("./dados/Rdata/glmm_l_md_sem_land_nCong.Rdata")
l_md2 <- get(l_md2)
l_md2 <- llply(l_md2,f_refit)
##
...
# 
oname <- load("./dados/Rdata/glmm_nCongcom_forestsuc_.Rdata")
l_md <- get(oname)
l_md %>% names()
l_md <- c(l_md,l_md2)
# l_md[[length(l_md)+1]] <- update(l_md[[length(l_md)]],. ~ . - k_z - land_hyp - k_z:land_hyp - (land_hyp|SiteCode) + 1 + (1|SiteCode))
# names(l_md)[length(l_md)] <- "1"
df_data <- df_plot
registerDoMC(2)
l_md <- llply(l_md,f_refit,.parallel = TRUE)
save(l_md,file="./dados/Rdata/glmm_nCongcom_forestsuc_.Rdata")
df_ts <- l_md %>% f_ts_R2()
write_csv(df_ts,file="./dados/csv/df_ts_defesa_congruencias.csv")
df_ts <- read.csv(file="./dados/csv/df_ts_defesa_congruencias.csv")
#
# criação das predições
l_md_e_start <- l_md[[df_ts$modelo[1]]]
save(l_md_e_start,file="./dados/Rdata/glmm_nCong_selecionado.Rdata")
```

```{r figura do logito predito para nCong e os 3 tipos de }
...
# f_pred <- \(md_e_starts,newdata){
#   ss <- md_e_starts$starts
#   md <- md_e_starts$modelo
#   f_predicaoBootStrap(lme=md,df_newdat = newdata)
# }  
# df_newpred <- f_pred(l_md_e_start,df_newdata)
...
```{r diagnosticos do modelo mais plausivel}
oname <- load(file="./dados/Rdata/glmm_nCong_selecionado.Rdata",verbose = TRUE)
l_md_e_start <- get(oname)
p_plot <- simulateResiduals(l_md_e_start$modelo,n = 250)
plot(p_plot)
o_testOut <- testOutliers(l_md_e_start$modelo,type="bootstrap")
save(p_plot,o_testOut,file="./apendices/analise_dados/Rdata/diagDHarma_md_nCong.Rdata")
```
...
p <- df_plot %>% 
  ggplot(aes(x=p,y=nCong,color=land_hyp)) +
  geom_smooth(se=FALSE,alpha=0.2,method="glm") + 
  geom_point() +
  facet_wrap(~k_f+forest_succession) +
  scale_color_manual(values=c("darkred","darkblue","darkgreen"),
                     labels=c("contemporâneo","idealizado","sem fragmentação")) + 
  labs(color="landscape hyp.") + 
  theme(legend.position = "top")
ggsave("./secoes_texto/Resultados/figuras/proposta_figura_final.png",
       plot = p,
       width = 15,height = 20)
```
...
l_md.logOR <- readRDS("dados/Rdata/l_md_logOR_aud.rds")
...
df_tabelaSelecao
write_csv(df_tabelaSelecao,file="dados/csv/df_tabelaSelecao_logOR_aud.csv")
#
...
```{r}
for(i in names(l_md_1alike)){
  md <- l_md_1alike[[i]]
  print(i)
  print(k.check(md))
  print(summary(md))
  print(f_MoranTest_GAMM(md))
