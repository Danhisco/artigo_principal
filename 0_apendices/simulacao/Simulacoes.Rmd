...
df_SoE <- read_csv("./dados/csv_SoE/df_plotSoE.csv") %>% 
  rename(SoE=2) %>% 
  mutate(SoE=ifelse(k>=0.50,2,SoE),
         k = round(k,2))
...
#
lpaths <- list.files(path="../csv_SoE2",
                     pattern = ".csv",
                     recursive = TRUE,
                     all.files = TRUE,
                     full.names = TRUE,
  ...
  gsub("../csv_SoE2/","",.) %>% 
  gsub("taxaU/MNEE/","",.) %>% 
  gsub("SADs_neutras/MNEE/","",.) %>% 
  gsub("cont|non_frag|ideal","",.) %>% 
  gsub("/","",.) %>% 
  gsub("_SoE2km.csv","",.)
vsites2 <- table(lpaths)
vsites_completos <- names(vsites2)[vsites2==6]
#
vsites <- setdiff(unique(df_sim$SiteCode),vsites_completos)
# df_sim2 <- filter(df_sim,!SiteCode%in%df_lf$SiteCode)
df_sim2 <- filter(df_sim,SiteCode%in%vsites)
