...
```{r}
setwd("../..")
# remoção dos casos de SoE = 1km
lf <- list.files(path="dados/csv_SoE",
                 pattern="1km",
                 full.names=TRUE,
                 recursive=TRUE)
file.remove(lf)
# junção dos SoE=2km de MNEE e de p_consolidar
## taxa U
lf <- list.files(path="dados/csv_SoE/taxaU/MNEE",
                 pattern="2km",
                 full.names=TRUE,
                 recursive=TRUE)
f_juntar <- \(lfi){
  dfx <- read_csv(lfi)
  vpath <- gsub("MNEE","p_consolidar",lfi)
  if(!file.exists(vpath)){
    ...
    dfr <- rbind(dfx,dfy) %>% 
      arrange(k) %>% 
      distinct()
    write_csv(dfr,file=lfi)
  }
}
lapply(lf,f_juntar)

## SADs
lf <- list.files(path="dados/csv_SoE/SADs_neutras/MNEE",
                 pattern="2km",
                 full.names=TRUE,
                 recursive=TRUE)
f_juntar <- \(lfi){
  ...
  if(!file.exists(vpath)){
    ...
    dfy <- read_csv(vpath)
    #
    if(length(vk)==14) dfx <- filter(dfx,k<=0.31)
    #
    dfr <- rbind(dfx,dfy) %>% 
      arrange(k) %>% 
      distinct()
    out <- write_csv(dfr,file=lfi)  
    rm(out,dfr,vk,dfx,dfy);gc()
  }
}
lapply(lf,f_juntar)
...
```{r roda o sumário dos resultados em função das paisagens hipotéticas}
...
df_contrastes <- read_csv(file="dados/csv_SoE/taxaU/df_contrastes.csv") %>% 
  ...
  pivot_longer(cols = c("area","frag.perse","frag.total"),
               values_to = "Uefeito",
               names_to = "contraste") %>% 
  mutate(par = case_when(contraste=="area" ~ "non_frag-ideal",
                         contraste=="frag.total" ~ "cont-ideal",
                         contraste=="frag.perse" ~ "cont-non_frag")) %>% 
  relocate(par,.after=contraste)
# df_logOR para modelar
df_logOR <- df_ad %>% 
  f_prep_contrasteSAD(.,dfUefeito=df_contrastes,pares=v_pares) %>% 
  inner_join(.,df_dados_disponiveis) %>% 
  filter(forest_succession!="capoeira") %>% 
  mutate(contraste =  gsub("area","Área per se",contraste) %>% 
           gsub("frag.perse","Frag. per se",.) %>% 
           gsub("frag.total","Frag. total",.),
         across(c(SiteCode,par,contraste,forest_succession),factor),
         k = as.numeric(as.character(k)))
saveRDS(df_logOR,file="dados/csv_SoE/df_logOR.rds")
write_csv(df_logOR,file="dados/csv_SoE/df_logOR.csv")
```

## Figuras complementares

```{r}
source("0_apendices/analise_dados/figcomp_resultados.R")
```

INCLUIR: criação do apêndice com as SAD simuladas

## Ajuste HGAM e diagnóstico

```{r ajuste HGAMs}
source("./0_apendices/analise_dados/ajuste_GAMM.R")
```

```{r tabela de seleção e diagnóstico do HGAM mais plausível}
...
# cria a predição a posteriori dos modelos mais plausíveis
source("./0_apendices/analise_dados/intervPred_GAMM.R")
# cria o regão de buffer
source("./0_apendices/analise_dados/regiao_buffer_figfinal.R")
```
