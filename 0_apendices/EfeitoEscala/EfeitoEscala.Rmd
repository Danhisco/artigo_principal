...
l_p[[2]] <- dfp0 %>% 
  ...
  geom_line(group=1) +
  geom_hline(yintercept = vxout,color="red") +
  geom_label(aes(label=paste0("aprox: ",vapprox),x=5,y=vxout)) +
  labs(x="lado da paisagem (km)",y="diferença acumulada entre lados da paisagem",
       title="b)") +
  # scale_y_continuous(labels = scales::scientific_format(digits = 2)) +
  facet_wrap(~k_factor,ncol=4) +
...
a <- 0.5
p <- df_plot |> 
  ggplot(aes(x=lado_numeric,y=Umed)) + #,group=k_factor,color=k
  geom_point(alpha=0.70,aes(color=S.N)) +
  ...
  stat_summary(fun = mean,
               ...
               geom = "pointrange",
               aes(x=lado_numeric,y=predict,color="avg+-sd"),alpha=0.6,size=a) +
  geom_vline(aes(xintercept=SoE_0.9,color=names(cols)[2]),alpha=0.5) +
  ...
  labs(title = "Média da estimativa média da taxa U predito pelo LMM:",
       subtitle="Umed ~ log(S) * log(N) * k * scale + (1|Site)",
  ...
  theme(legend.position = "top",
        strip.text = element_text(size = 10,margin = margin()),
        panel.spacing = unit(3, "pt"))
saveRDS(p,file="0_apendices/EfeitoEscala/figuras/fig5_SoE.rds")
vpath <- ggsave("0_apendices/EfeitoEscala/figuras/fig5_SoE.png",plot = p,
       width = 10.2,
       height = 6.7)
library(magick)
f_img_trim_resize(vpath)
...
p <- df_aud %>% 
  ...
  labs(x="log(N)",y="S") +
  theme_bw()
v_path <- ggsave("0_apendices/EfeitoEscala/figuras/figSI_S_N_sizeDA_fillSN.png",p)
f_img_trim_resize(v_path)
```
