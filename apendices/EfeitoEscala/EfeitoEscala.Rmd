...
df_md <- df_Usoe |> 
  ...
  select(SiteCode,Umed,logit_Umed,k_factor,lado_factor,lado_km,log2_lado.km,S_obs,Ntotal,DA) |> 
  mutate(across(S_obs:DA,log,.names="log_{.col}"),
         across(S_obs:log_DA,f_z,.names="{.col}_z"))
```
...
l_md[[2]] <- lmer(logit_Umed ~ log_S_obs_z * log_Ntotal_z * k_factor * poly(log2_lado.km,4,raw = TRUE) + (1|SiteCode),
                  data=df_md)
names(l_md) <- c("k * lado factor + 1|Site",
                 "k * lado poly4 + 1|Site")
...
__Tabela 1__ Coeficientes de determinal (R2) marginal(m, desconsiderando o efeito aleatório) e condicional(c, considerando o efeito aleatório):

```{r R2 marginal e condicional do modelo mais plausível para descrever SoE}
# print("R2 marginal e condicional, respectivamente:")
AICctab(l_md,weights=T) |> as.data.frame() |> head(n=1) |> row.names() |> 
  (\(.) MuMIn::r.squaredGLMM(l_md[[.]]))()
```
...
df_newdata$Umed_predict <- predict(md_SoE,df_newdata)
df_newdata <- df_newdata |> 
  mutate(Umed = exp(Umed_predict) / exp(1-Umed_predict))
# gráficos
df_newdata |> 
