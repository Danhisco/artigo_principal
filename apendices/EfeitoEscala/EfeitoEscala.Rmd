...
df_U$k_factor <- factor(df_U$k,levels = sort(unique(df_U$k),decreasing = T))
# auditoria
df_U |> filter(Ulag < 0) |> arrange(Ulag) |> count(k) |> ggplot(aes(x=k,y=n)) +
  geom_point() +
  labs(title="Freq. de Ulag<0 por k")
# df_U |> filter(Ulag < 0) |> arrange(Ulag) |> head()
# df <- df_U |> filter(SiteCode == "MGvico3" & round(k,2) == 0.65)
df_U |> 
  group_by(SiteCode,k) |> 
  summarise(Ucv = sd(Umed) * 100 / mean(Umed)) |> 
  mutate(k_factor = factor(round(k,2))) |> 
  ggplot(aes(x=k_factor,y=Ucv)) +
  geom_jitter() +
  geom_boxplot()
```
