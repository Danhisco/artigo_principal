...
A estimativa de U depende da arvóre genalógica da comunidade simulada por MNEE. O número de indivíduos (N ou DA) determina o número inicial de linhagens que são monitoras e o número de espécies (S) determina o número final de linhagens monitoradas. A árvore genealógica da comunidade também é influenciada pelo grau de limitação de dispersão (k) e da extensão espacial (scale), pois em alguns graus de limitação de dispersão a árvore genealógica pode ser restringida pela extensão da paisagem e não pela limitação de dispersão. 

__Modelos Estatísticos__

Para descrever a média da estimativa da taxa U optamos por transformar a variável usando a função logito e usar um modelo gaussiano misto (LMM). Primeiro selecionaremos variáveis de controle (tabela 1). Então selecionaremos a maneira mais plausível de descrever a relação entre escala e grau de limitação de dispersão (k e scale, respectivamente, como fatores).

__tabela 1__ Seleção das variáveis de controle. S_obs = riqueza observada, N = número de indivíduos, DA = densidade de indivíduos; algumas variáveis estão na escala log e todas as variáveis estão z transformadas. Funções lmer do pacote lme4 (REF), AICctab do pacote bblme (REF). 
...
l_md[[2]] <- lmer(logit_Umed ~ log_S_obs_z * log_DA_z * k_factor * lado_factor + (1|SiteCode),data=df_md)
l_md[[3]] <- lmer(logit_Umed ~ log_S_obs_z * k_factor * lado_factor + (1|SiteCode),data=df_md)
l_md[[4]] <- lmer(logit_Umed ~ S_obs_z * Ntotal_z * k_factor * lado_factor  + (1|SiteCode),data=df_md)
l_md[[5]] <- lmer(logit_Umed ~ S_obs_z * DA_z * k_factor * lado_factor + (1|SiteCode),data=df_md)
l_md[[6]] <- lmer(logit_Umed ~ S_obs_z * k_factor * lado_factor + (1|SiteCode),data=df_md)
names(l_md) <-laply(l_md,f_nameModel)
AICctab(l_md,weights=T)
```

O modelo mais plausível considera a interação dos logs da riqueza observada e número de indivíduos na parcela (tabela 1).
...
l_md[[1]] <- lmer(logit_Umed ~ log_S_obs_z * log_Ntotal_z * k_factor * lado_factor + (1|SiteCode),
                  data=df_md)
l_md[[2]] <- lmer(logit_Umed ~ log_S_obs_z * log_Ntotal_z * k_factor * poly(log2_lado.km_z,4,raw = TRUE) + (1|SiteCode),
                  data=df_md)
names(l_md) <- c("k * lado factor + 1|Site","k * lado poly4 + 1|Site")
AICctab(l_md,weights=T)
```
...
__Figura 5__ Resíduos quantilícos do LMM mais plausível: logit(Umed) ~ log(S) + log(N). Umed = média da bateria de simulações (# réplicas = 10), S = riqueza de espécies, N = número de indivíduos.
