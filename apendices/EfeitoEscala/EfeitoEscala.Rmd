...
#### 3o Passo
##### Calcular o peso de evidência por lado_km

```{r 3o passo,results='hide'}
teste <- MuMIn::Weights(MuMIn::AICc(l_md))
...
(raio_medio <- round(weighted.mean(x = df_averageSE$lado_km,w = df_averageSE$weight),1))
df_averageSE |> filter(weight==max(df_averageSE$weight))
```
...
l_p[[3]] <- df_se |> 
  # arrange(factor(SiteCode,levels=v_levels)) |> 
  ggplot(aes(x=SiteCode,y=p)) +
  geom_point(aes(color=lado_km),alpha=0.3) +
  scale_x_discrete(limits=v_levels)
grid.arrange(grobs=l_p[2:1],ncol=1,
