...
df_Usoe$lado_factor <- factor(df_U$lado_km,labels=c(paste(0.5,"->",16/(2^(4:0))),NA))
df_Usoe$k_factor <- factor(df_U$k,levels = sort(unique(df_U$k),decreasing = T))
# df_resultadoSoE
f_ddply <- function(df){
  df |> 
    filter(Ucsum >= 0.9) |> 
...
df_soe90 <- ddply(df_U,c("SiteCode","k"),f_ddply)
df_aud <- anti_join(df_sorteio_sitios,df_resultadoSoE,by=c("SiteCode","k")) |> 
  inner_join(df_U,by=c("SiteCode","k")) |> 
  select(-(txt.file:DA))
```
...
<p> __Figura 4__ O efeito de escala acumulado entre escalas consecutivas. 
