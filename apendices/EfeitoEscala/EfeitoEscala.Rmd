...
df_se <- df_se_completo |> 
  filter(lado_km == 16.02) |> arrange(p) |> 
  inner_join(select(df_dados,SiteCode,effort_ha,tif.path),by="SiteCode") |> 
  mutate(tif.path = paste0("dados/paisagens_mapbiomas6_padronizadas//",SiteCode,".tif"),
         DA = Ntotal/effort_ha) |>
  filter(p > 0.99 | p < 0.073) |> head()
# df_auditoria dos recortes de paisagens padronizados para MNEE na
df_auditoria <- data.frame(txt.path = list.files(path = "dados/simulacao",pattern = ".txt",full.names = T)) |> 
  ...
  inner_join(select(df_se,-tif.path),by = "SiteCode")
# df dados para simulação
# df_sim <- read_csv("dados/df_simulacao.csv") |> 
#   left_join(x=df_se,y=.,by=SiteCode)
