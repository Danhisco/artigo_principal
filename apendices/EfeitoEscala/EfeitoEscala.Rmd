...
df_newdata <- df_sim |> 
  inner_join(y=distinct(select(df_Usoe, k,k_factor,lado_km)),"k") |> 
  select(k_factor, lado_km, Ntotal, S_obs) |>
  mutate(across(Ntotal:S_obs,log,.names="log_{.col}"),
         across(Ntotal:log_S_obs,f_z,.names="{.col}_z"),
         lado_factor = factor(round(lado_km,1)),
         SiteCode = df_md$SiteCode[1]) #|> 
  # select(SiteCode,k_factor,lado_factor,log_Ntotal_z:log_S_obs_z)
...
df_newdata |> 
  ...
  pivot_longer(cols=c(Umed,Umed_cFiltro)) |> 
  ggplot(aes(x=lado_factor,y=value,color=name,group=name)) + #,group=k_factor,color=k
  # stat_summary(fun.data = median,geom = "line",aes(y=value,group=name)) +
  stat_summary(aes(x=lado_factor,y=value,color=name,group=name),
