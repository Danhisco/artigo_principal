...
<p> Primeiro avaliamos se a maior extensão considerada é suficiente para obter o padrão não linear de U esperado em paisagens infinitas. E depois comparamos a estimativa de U para cada subescala. Dos 108 sítios selecionados para a comparação dos modelos neutros, 36 foram sorteados e selecionados para avaliar qual a extensão espacial suficiente para comparar os modelos neutros. Esses 36 sítios incluem os extremos da riqueza de espécies e densidade de indivíduos observada e outros 32 sítios sorteados na amplitude de valores dessas duas variáveis (figura 1a). Para cada sítio simulamos 20 graus de limitação de dispersão no modelo espacialmente explícito (Figura 1b e Figura 2). As subescalas avaliadas variaram de 0.5 km até 16 km de lado, totalizando 6 escalas espaciais da paisagem ao redor. Vamos selecionar os graus de limitação de dispersão no qual a extensão espacial de 4x4 km2 acumula a maior parte do efeito da escala na estimativa de U . </p>
...
A taxa U estimada na extensão de 16x16 km2 apresenta padrão não linear com um máximo em graus pouco severos de limitação de dispersão (Figura 2). Há muita variação entre os sítios, relacionado com a riqueza de espécies (texto principal resultados), por isso na figura 2 o eixo y varia entre paineis. A variação entre réplicas de uma mesma bateria de simulação pode ser elevada em graus de limitação de dispersão pouco severos (Figura 2). Os valores médios sugerem um padrão de patamares, com valores baixos de U quando a limitação de dispersão é severa e em graus pouco severos um aumento brusco que suavemente diminui com o aumento do relaxamento da limitação de dispersão (Figura 2).
...
Principalmente para os graus de limitação de dispersão brandos, observamos o padrão esperado de a taxa U diminuir com o aumento da escala (Figura 3). Em alguns graus de limitação de dispersão há pouca variação entre escalas (Figura 3 e 4), para graus pouco severos de limitação de dispersão o efeito da escala pode ser menor do que para graus severos (Figura 3 e 4). Os graus pouco severos devem operar no máximo global de taxa U necessária para obter a riqueza observada (Figura 2) e por isso devem apresentar baixa variação na estimativa da taxa U. 
...
Calculamos a diferença entre a média de U de escalas consecutivas e dividimos pela amplitude de U para todas as escalas (Ulag). Eperavamos que o aumento da escala iria reduzir a estimativa da taxa U, ou seja, os valores extremos de escala seriam os valores extremos de U e dessa forma seria possível quantificar o efeito acumulado até uma determinada escala. Isso pode ser observado em nos graus de limitação de dispersão severos e principalmente nos brandos (Figura 5), porém em geral nos graus pouco severos esse padrão pode não ocorrer, e valores negativos de Ulag podem ser observados (Figura 5).
...
__Figura 5__ Ulag para todas as escalas (eixo y) pela grau de limitação de dispersão (eixo x), linha vermelha marca o valor zero.

Valores de Ulag negativos indicam que o aumento da escala aumentam a estimativa média de U, isso pode ocorrer pois há estabilização na estimativa média de U ou que há aumento consistente. 

```{r}
df_plot <- df_Usoe |> filter(Ulag==-1) |> select(SiteCode, k) |> 
  inner_join(df_Usoe.rep,by=c("SiteCode","k")) |> 
  mutate(DA = Ntotal / effort_ha,
         k = round(k,2),
         k_factor = factor(k),
         label = paste0(SiteCode,", S=",S_obs,", k=",k_factor)) 
df_plot$label <- factor(df_plot$label,levels = unique(df_plot$label))
df_plot |>
  ggplot(aes(x=lado_km,y=U,color=k,group=factor(k))) +
  geom_point() +
  ...
  theme_gray() +
  # theme(legend.position = "bottom") +
  labs(title = "Casos em que Ulag =-1",
