...
```{r antiga figura 3 Umed por lado da paisagem,fig.width=13,fig.height=8}
df_plot <- df_Usoe.rep |> 
...
```{r figura 7 Ucumulative,fig.height=8,fig.width=8}
df_Usoe |> 
  drop_na() |>
...
```{r figura para detalhes de Ulag}
df_Usoe |>
  drop_na() |> 
  pivot_longer(Ulag:Ulag0) |>
  ggplot(aes(x=Schange,y=value, color=name)) +
  geom_jitter() +
  geom_boxplot() +
  scale_colour_manual(values=c("black","green")) +
  geom_hline(yintercept = c(0,0.1), color="red") +
  labs(x="mudança de escala",y="(U_i - U_i+1) / diff(range(U))",color="") +
  facet_wrap(~k_factor, nrow=4) +
  theme(legend.position = "top")
df_Usoe |> 
  drop_na() |> 
  pivot_longer(Ucsum:Ucsum0) |>
  ggplot(aes(x=Schange_cumulative,y=value, color=name)) +
  geom_jitter() +
  geom_boxplot() +
  scale_colour_manual(values=c("black","green")) +
  geom_hline(yintercept = c(0,0.9), color="red") +
  facet_wrap(~k_factor, nrow=4)
```
