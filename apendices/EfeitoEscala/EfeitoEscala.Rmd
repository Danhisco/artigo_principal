...
df_md <- df_Usoe |> 
  ...
  select(SiteCode,Umed,logit_Umed,k_factor,lado_factor,lado_km,log2_lado.km,S_obs,Ntotal,DA,S.DA) |> 
  mutate(across(S_obs:S.DA,log,.names="log_{.col}"),
         across(S_obs:log_S.DA,f_z,.names="{.col}_z"))
# dados completos para serem preditos - não usado pois há mais valores com S/DA elevado do que na base simulada
md_SoE <- lmer(logit_Umed ~ log_S_obs_z * log_Ntotal_z * k_factor * lado_factor + (1|SiteCode),
                  data=df_md)
df_newdata <- df_sim |> 
  inner_join(x=distinct(select(df_Usoe, k,k_factor,lado_km)),"k") |> 
  select(k,k_factor, lado_km, Ntotal, S_obs) |>
  mutate(across(Ntotal:S_obs,log,.names="log_{.col}"),
         across(Ntotal:log_S_obs,f_z,.names="{.col}_z"),
         lado_factor = factor(round(lado_km,1)),
         SiteCode = df_md$SiteCode[1])
df_newdata$Umed <- exp(predict(md_SoE,df_newdata)) / exp(1-predict(md_SoE,df_newdata))
# dados de dados usados na simulação com os valores preditos e os residuos z transformados
df_plot <- df_md |> 
...
__1) descrever o padrão por modelos de regressão GLMM e GAMM__
__2) aplicar a lógica do Ulag -> Ucsum no padrão predio pelas análises de regressão__

Procuro considerar todos os dados sem aplicar filtros que modificam a variabilidade dos dados, além de poder descrever o efeito de escala por grau de limitação de dispersão. Proposta: i) selecionar as variáveis padronizadoras (riqueza de espécies, S; número de indivíduos, N; densidade de indivíduos)  

__Introdução__

A estimativa de U depende da arvóre genalógica da comunidade simulada por MNEE, o número de indivíduos (N ou DA) determina o número inicial de linhagens que serão monitoras e o número de espécies (S) determina o número final de linhagens monitoradas. A árvore genealógica da comunidade também é influenciada pelo grau de limitação de dispersão e da extensão espacial, pois em alguns graus de limitação de dispersão a árvore genealógica pode ser restringida pela extensão da paisagem.   


e portanto a interação entre elas também é plausível a priori. 
Primeiro irei selecionar 


```{r GLMM selecao de variaveis para fixar no GLMM final,eval=T,cache=T}
# modelos
l_md <- list()
l_md[[1]] <- lmer(logit_Umed ~ log_S_obs_z * log_Ntotal_z + (1|SiteCode),data=df_md)
l_md[[2]] <- update(l_md[[1]], . ~ . - log_S_obs_z * log_Ntotal_z + log_S_obs_z * log_DA_z)
l_md[[3]] <- update(l_md[[1]], . ~ . - log_S_obs_z * log_Ntotal_z + log_S_obs_z)
l_md[[4]] <- lmer(logit_Umed ~ S_obs_z * DA_z + (1|SiteCode),data=df_md)
l_md[[5]] <- update(l_md[[4]], . ~ . - S_obs_z * DA_z + S_obs_z * Ntotal_z)
l_md[[6]] <- update(l_md[[4]], . ~ . - S_obs_z * DA_z + S_obs_z)
l_md[[7]] <- lmer(logit_Umed ~ poly(log_S.DA_z,2,raw = TRUE) + (1|SiteCode),data=df_md)
l_md[[8]] <- lmer(logit_Umed ~ poly(S.DA_z,2,raw = TRUE) + (1|SiteCode),data=df_md)
l_md[[9]] <- lmer(logit_Umed ~ 1 + (1|SiteCode),data=df_md)
names(l_md) <-laply(l_md,f_nameModel)
df_AICctab <- AICctab(l_md,weights=T) |> as.data.frame() #|> head(n=1) |> row.names()
df_AICctab
md_pGAMM <- l_md[[row.names(df_AICctab[1,])]]
# v_betas <- fixef(md_pGAMM)[-1] %>% as.numeric() %>% round(.,digits = 3)
```

```{r futura figura 5 - avaliação de modelo mais plausível md_pGAMM DHARMa residuals, eval=F}
p_plot <- simulateResiduals(md_pGAMM,n=1000)
plot(p_plot)
```

__Futura Figura 5__ Resíduos quantilícos do LMM mais plausível: logit(Umed) ~ log(S) + log(N). Umed = média da bateria de simulações (# réplicas = 10), S = riqueza de espécies, N = número de indivíduos.

```{r comparação do modelo mais plausivel desconsiderando logit Umed, eval=F, include=F}
l_md <- list()
l_md[[1]] <- lmer(Umed ~ log_S_obs_z * log_Ntotal_z + (1|SiteCode),data=df_md)
l_md[[2]] <- update(l_md[[1]], . ~ . - log_S_obs_z * log_Ntotal_z + log_S_obs_z * log_DA_z)
l_md[[3]] <- update(l_md[[1]], . ~ . - log_S_obs_z * log_Ntotal_z + log_S_obs_z)
l_md[[4]] <- lmer(Umed ~ S_obs_z * DA_z + (1|SiteCode),data=df_md)
l_md[[5]] <- update(l_md[[4]], . ~ . - S_obs_z * DA_z + S_obs_z * Ntotal_z)
l_md[[6]] <- update(l_md[[4]], . ~ . - S_obs_z * DA_z + S_obs_z)
names(l_md) <-laply(l_md,f_nameModel)
df_AICctab <- AICctab(l_md,weights=T) |> as.data.frame() #|> head(n=1) |> row.names()
md_pGAMM_Umed <- l_md[[row.names(df_AICctab[1,])]]
p_plot <- simulateResiduals(md_pGAMM_Umed,n=1000)
plot(p_plot)
```


#### efeito de escala: interação entre extensão espacial e grau de limitação de dispersão

```{r GLMM para avaliação de efeito de escala,echo=T}
l_md <- list()
l_md[[1]] <- lmer(logit_Umed ~ log_S_obs_z * log_Ntotal_z * k_factor * lado_factor + (1|SiteCode),
                  data=df_md)
l_md[[2]] <- lmer(logit_Umed ~ log_S_obs_z * log_Ntotal_z * k_factor * poly(log2_lado.km,4,raw = TRUE) + (1|SiteCode),
                  data=df_md)
l_md[[3]] <- lmer(logit_Umed ~ poly(log_S.DA_z,3,raw = TRUE) * k_factor * lado_factor + (1|SiteCode),
                  data=df_md)
l_md[[4]] <- lmer(logit_Umed ~ poly(log_S.DA_z,3,raw = TRUE) *  k_factor * poly(log2_lado.km,4,raw = TRUE) + (1|SiteCode),
                  data=df_md)
names(l_md) <- c("S * N * k * lado factor + 1|Site","S * N * k * lado poly4 + 1|Site",
                 "S.DA * lado factor + 1|Site","S.DA * k * lado poly4 + 1|Site")
...
```{r comparacao possiveis submodelos para descrever o efeito de escala, echo=TRUE}
l_md <- list()
l_md[[1]] <- lmer(logit_Umed ~ log_S_obs_z * log_Ntotal_z * k_factor * lado_factor + (1|SiteCode),
                  data=df_md)
l_md[[2]] <- lmer(logit_Umed ~ log_S_obs_z * log_Ntotal_z + k_factor * lado_factor + (1|SiteCode),
                  data=df_md)
l_md[[3]] <- lmer(logit_Umed ~ log_S_obs_z + log_Ntotal_z + k_factor * lado_factor + (1|SiteCode),
                  data=df_md)
l_md[[4]] <- lmer(logit_Umed ~ log_S_obs_z + k_factor * lado_factor + (1|SiteCode),
                  data=df_md)
l_md[[5]] <- lmer(logit_Umed ~ k_factor * lado_factor + (1|SiteCode),
                  data=df_md)
l_md[[6]] <- lmer(logit_Umed ~ log_S_obs_z * log_Ntotal_z * k_factor + lado_factor + (1|SiteCode),
                  data=df_md)
l_md[[7]] <- lmer(logit_Umed ~ log_S_obs_z * log_Ntotal_z + k_factor + lado_factor + (1|SiteCode),
                  data=df_md)
l_md[[8]] <- lmer(logit_Umed ~ log_S_obs_z + log_Ntotal_z + k_factor + lado_factor + (1|SiteCode),
                  data=df_md)
l_md[[9]] <- lmer(logit_Umed ~ log_S_obs_z + k_factor + lado_factor + (1|SiteCode),
                  data=df_md)
l_md[[10]] <- lmer(logit_Umed ~ k_factor + lado_factor + (1|SiteCode),
                  data=df_md)
names(l_md) <- c("* * *", "* + *", "+ + *", "S + *","0 + *",
                 "* * +", "* + +", "+ + +", "S + +","0 + +")
AICctab(l_md,weights=T)
...
__Tabela 1__ Coeficientes de determinal (R2) do modelo mais plausível: marginal(m, desconsiderando o efeito aleatório) e condicional(c, considerando o efeito aleatório):
...
df_ranef |>
  pivot_longer(Ntotal:S_obs) |> head()
  ggplot(aes(sample=parameter_value)) + 
  stat_qq(alpha=0.3,aes(color=log(S_obs))) + stat_qq_line(alpha=0.3,col="red") + theme_classic() +
  facet_wrap(~parameter_class,scales="free",ncol=3)
df_ranef |> 
  ggplot(aes(x=log(S_obs),y=parameter_value)) +
  geom_point() +
  geom_smooth()
```
...
```{r diagnosticos 2 conjunto de dados totais }
# modelo mais plausível
# gráficos
l_p <- list()
l_p[[1]] <- df_newdata |>
  # filter(S_obs < 250, Ntotal < 5000) |>
  mutate(k = as.numeric(as.character(k_factor))) |> 
  ggplot(aes(x=lado_factor,y=Umed)) + #,group=k_factor,color=k
  stat_summary(fun.data = mean_se) +
  stat_summary(fun.data = mean_se,geom = "line",aes(group=k_factor)) +
  facet_wrap(~k_factor,ncol=5) +
  labs(title="sem filtro")
l_p[[2]] <- df_newdata |>
  filter(S_obs < 250, Ntotal < 5000) |>
  mutate(k = as.numeric(as.character(k_factor))) |> 
  ggplot(aes(x=lado_factor,y=Umed)) + #,group=k_factor,color=k
  stat_summary(fun.data = mean_se) +
  stat_summary(fun.data = mean_se,geom = "line",aes(group=k_factor)) +
  labs(title="S < 250 | N < 5000") +
  facet_wrap(~k_factor,ncol=5)
grid.arrange(grobs=l_p,ncol=2)
df_newdata |> 
  mutate(filtro = ifelse(S_obs>250 | Ntotal>5000,NA,1),
         Umed_cFiltro = Umed * filtro,
         label=paste0(S_obs,", ",Ntotal)) |> 
  # filter(is.na(filtro)) |> pull(label) |> unique() |> length() # apenas 6 combinações de S e Ntotal
  pivot_longer(cols=c(Umed,Umed_cFiltro)) |> 
  ggplot(aes(x=lado_factor,y=value,color=name,group=name)) + #,group=k_factor,color=k
  stat_summary(fun.data = mean_se) +
  stat_summary(fun.data = mean_se,geom = "line",aes(group=name)) +
  # labs(title="S < 250 | N < 5000") +
  facet_wrap(~k_factor,ncol=5)
df_newdata |>
  ggplot(aes(x=S_obs,y=Umed, color=k)) +
  geom_point() +
  geom_smooth() +
  facet_wrap(~k_factor,ncol=5,scales="free")
df_newdata |> 
  ggplot(aes(x=k_factor,y=Umed)) +
  geom_jitter() +
  geom_boxplot()
df_sim |> 
  ggplot(aes(x=S_obs,y=Ntotal)) +
  geom_point()
df_newdata |>
  filter(S_obs >= 250 | Ntotal >= 5000) |> 
  group_by(S_obs,Ntotal) |>
  mutate(label=factor(paste0(S_obs,", ",Ntotal))) |> 
  ggplot(aes(x=lado_factor,y=Umed,group=label, color=label)) +
...
df_newdata |> 
  ...
  mutate(label=factor(paste0(S_obs,", ",Ntotal)),
         S.DA = S_obs / Ntotal) |> 
  ungroup() |> 
  # filter(S.DA <= quantile(S.DA,probs=0.90)) |> 
  ggplot(aes(x=lado_factor,y=Umed,group=label, color=S.DA)) +
  geom_point(aes(group=label)) +
  geom_line(aes(group=label)) +
  scale_color_distiller(palette="Spectral") +
  labs(color="S / N",
       title="Umed > 0.9 quantile") +
  facet_wrap(~k_factor)
df_newdata |> 
  group_by(S_obs,Ntotal) |>
  mutate(label=factor(paste0(S_obs,", ",Ntotal)),
         S.DA = S_obs / Ntotal) |> 
  ungroup() |> 
  ggplot(aes(x=S.DA,y=Umed,color=lado_km)) +
  geom_point() +
  geom_smooth(se=F) +
  facet_wrap(~k_factor)
```
...
df_plot |> 
  ...
  stat_summary(fun.data = mean_se,geom = "line",color="red",aes(group=k_factor)) +
  stat_summary(aes(x=lado_factor,y=Umed), #,color=name,group=name
               fun.min = \(.) quantile(.,0.25),
               fun.max = \(.) quantile(.,0.75),
               fun = median,color="blue") +
  stat_summary(aes(x=lado_factor,y=Umed,group=k_factor),
               fun = median,
...
df_Ulag <- df_newdata |> 
  group_by(k_factor,lado_factor) |> 
  summarise(Umed_median = median(Umed),
            Umed_1q = quantile(Umed,0.25),
            Umed_3q = quantile(Umed,0.75)) |> 
  ungroup() |> 
  pivot_longer(Umed_median:Umed_3q,values_to = "Umed") 
df_plot <- ddply(df_Ulag,c("k_factor","name"),f_lag.U)
df_plot |> 
  drop_na() |> 
  filter(name=="Umed_median",Ucsum>=0.95) |>
  ddply(.variables = "k_factor",.fun = \(x) head(x,n=1))
  
```



```{r alternaivamenet poderíamos considerar as réplicas para uilizar GAMM?,eval=F,include=F}
# para utilizar GAMM talvez seja necessário considerar as réplicas
# md_GAMM <- gam(logit_Umed ~ offset(v_betas[1] * log_S_obs_z) + offset(v_betas[2] * log_Ntotal_z) +
#                  s(log2_lado.km,by=k_factor,k = 2) + s(SiteCode,bs="re"),
               # data=df_md)
```
