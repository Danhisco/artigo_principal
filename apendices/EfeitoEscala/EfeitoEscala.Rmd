...
df_md <- df_Usoe |> 
  ...
  select(SiteCode,Umed,logit_Umed,k_factor,lado_km,log2_lado.km,S_obs,Ntotal,DA) |> 
...
l_md[[2]] <- update(l_md[[1]], . ~ . - log_Ntotal_z + log_DA_z)
l_md[[3]] <- update(l_md[[1]], . ~ . - log_Ntotal_z)
l_md[[4]] <- lmer(logit_Umed ~ S_obs_z + DA_z + (1|SiteCode),data=df_md)
l_md[[5]] <- update(l_md[[4]], . ~ . - DA_z + Ntotal_z)
l_md[[6]] <- update(l_md[[4]], . ~ . - DA_z)
...
```{r comparação do modelo mais plausivel desconsiderando logit Umed}
l_md <- list()
