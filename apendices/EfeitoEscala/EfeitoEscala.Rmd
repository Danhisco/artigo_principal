...
Procuro considerar todos os dados sem aplicar filtros que modificam a variabilidade dos dados, além de poder descrever o efeito de escala por grau de limitação de dispersão  

#### LMM: seleção de variáveis

__Objetivo__
selecionar quais as variáveis de controle que serão fixadas no GAMM: riqueza (S), número de indivíduos (N), densidade (DA) e suas versões em log. O modelo estatístico pressupõem relação linear dessas preditoras com o 


```{r GLMM selecao de variaveis para fixar no GAMM,eval=T,cache=T}
# modelos
l_md <- list()
...
```{r}
l_md <- list()
l_md[[1]] <- update(md_pGAMM,. ~ . + log2_lado.km)
l_md[[2]] <- update(md_pGAMM,. ~ . + poly(log2_lado.km,2,raw = TRUE))
l_md[[3]] <- update(md_pGAMM,. ~ . + poly(log2_lado.km,3,raw = TRUE))
names(l_md) <- paste("poly",1:3)
df_AICctab <- AICctab(l_md,weights=T) |> as.data.frame() #|> head(n=1) |> row.names()
md_GAMM <- l_md[[row.names(df_AICctab[1,])]]
