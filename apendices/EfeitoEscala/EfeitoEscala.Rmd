...
## df_Unull
df_Unull.rep <- list.files(path = "dados/csv/U_nullLand_16km/",pattern = ".csv",full.names = T) |> 
  map_df(~read_csv(.)) 
df_Unull <- df_Unull.rep |> 
  pivot_longer(cols=`1`:`4`, names_to = "n", values_to = "U") |> 
  group_by(SiteCode, k) |> 
  summarise(Umed = mean(U), Usd = sd(U)) |> 
  inner_join(x=distinct(select(df_sim,SiteCode,p:S_obs)),by="SiteCode") |> 
...
### Mapas de Cobertura Vegetal usados (4x4km2)
