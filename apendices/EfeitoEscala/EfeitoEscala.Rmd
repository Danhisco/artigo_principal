...
```{r}
l_md <- list()
l_md[[1]] <- lmer(logit_Umed ~ log_S_obs_z * log_Ntotal_z +
                    k_factor * poly(log2_lado.km,4,raw = TRUE) + (k_factor|SiteCode),
                  data=df_md)
l_md[[2]] <- lmer(logit_Umed ~ log_S_obs_z * log_Ntotal_z +
                    k_factor * poly(log2_lado.km,4,raw = TRUE) + (1|SiteCode),
                  data=df_md)
names(l_md) <- c("k|Site","1|Site")
df_AICctab <- AICctab(l_md,weights=T) |> as.data.frame() #|> head(n=1) |> row.names()
df_AICctab
md_SoE <- l_md[[row.names(df_AICctab[1,])]]
p_plot <- simulateResiduals(md_GAMM,n=1000)
plot(p_plot)
