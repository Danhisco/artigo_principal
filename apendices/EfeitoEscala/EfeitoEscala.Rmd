...
df_se <- data.frame(txt.file = list.files(path = "dados/simulacao",pattern = ".txt"), #,full.names = T
                    SiteCode = str_extract(list.files(path = "dados/simulacao",pattern = ".txt"), #,full.names = T
                                           ".*?(?=.txt)")) |> #(?<=ao\\/)
...
```{r figura 2 mapa das paisagens que foram usadas na estimativa de SoE por MNEE,fig.width=8,fig.height=6}
df <- df_se
...
```{r figura 3 detalhe da vizinhança da parcela,message=FALSE,warning=FALSE,fig.width=8,fig.height=6}
...
```{r figura 4 Umed por lado da paisagem,fig.width=8,fig.height=6}
# df_U <- cbind(df_Urep[,1:6],
#               Umed = apply(df_Urep[,7:11], 1, mean),
#               Ucv = apply(df_Urep[,7:11], 1, sd))
df_Urep |> 
  ...
  mutate(DA = Ntotal/effort_ha,
         label = paste(SiteCode,S_obs,Ntotal,DA,sep = ", ")) |> 
...
df_se1 <- df_se |> filter(!(SiteCode %in% unique(df_Urep$SiteCode)))
registerDoMC(3)
df_U <- adply(df_se1,1,f_SoE_MNEE,.id = "SiteCode",.parallel = TRUE)
df_Urep <- rbind(select(df_U,-tif.path),df_Urep)
registerDoMC(3)
write_csv(df_Urep,file="../csv/df_Urep.csv")
```
