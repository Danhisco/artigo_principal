...
l_md[[2]] <- glmer(cbind(nCongKS,100-nCongKS) ~ 
                   poly(p_z,2,raw=TRUE) * poly(k_cont_z,2,raw=TRUE) + 
                   (poly(k_cont_z,2,raw=TRUE)|SiteCode),
          data=df_ad, family = "binomial",
          control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e9)),na.action = "na.fail")
...
#           data=df_ad, family = "binomial",
#           control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e9)),na.action = "na.fail")
names(l_md) <- c("poly 3","poly 2", "poly 1")
...
```{r modelos cheios,echo=TRUE,cache=TRUE}
l_md <- list()
l_md[[1]] <- glmer(cbind(nCongKS,100-nCongKS) ~ 
                   (p_z + I(p_z^2) + I(p_z^3)) *
                  (k_cont_z + I(k_cont_z^2) + I(k_cont_z^3)) + 
                   (k_cont_z + I(k_cont_z^2) + I(k_cont_z^3)|SiteCode),
          data=df_ad, family = "binomial",
          control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e9)),na.action = "na.fail")
l_md[[2]] <- glmer(cbind(nCongKS,100-nCongKS) ~ 
                   (p_z + I(p_z^2)) *
                  (k_cont_z + I(k_cont_z^2)) + 
                   (k_cont_z + I(k_cont_z^2)|SiteCode),
          data=df_ad, family = "binomial",
          control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e9)),na.action = "na.fail")
l_md[[3]] <- glmer(cbind(nCongKS,100-nCongKS) ~ 
                   p_z * k_cont_z + 
          ...
          data=df_ad, family = "binomial",
          control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e9)),na.action = "na.fail")
names(l_md) <- c("poly 3|SiteCode", "poly 2|SiteCode", "poly 1|SiteCode")
#
# aictab(l_md)
#
```
...
l_md.dredge_MNEE <- llply(dredge(md_MNEE,trace = FALSE,evaluate=FALSE), eval,.parallel = TRUE)
  # llply(dredge(md_MNEE,trace = FALSE,evaluate=FALSE), eval,.parallel = TRUE)
