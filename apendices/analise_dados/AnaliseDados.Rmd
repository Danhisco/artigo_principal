...
### Predição por bootstrap

```{r predito boostrap MNEE }
#new data
df_newdat <- expand.grid(SiteCode=df_ad$SiteCode[1], 
                         ...
                         log_S_obs_z = seq(min(df_ad$log_S_obs_z),max(df_ad$log_S_obs_z), length=30),
                         log_Ntotal_z = seq(min(df_ad$log_Ntotal_z),max(df_ad$log_Ntotal_z), length=30))
...
b3 <- bootMer(md_MNEE, FUN = f1, nsim=1000)
