...
CPF:
1) protocolo de validação: 4 elementos - k.check, summary, appraise, draw
2) gráfico de 1 d sobrepondo novo conjunto de dados e o observado
...
f_mdGAMMcongCont <- \(df){
  gam(cbind(nCong,500-nCong) ~ 
        s(contraste_z) + 
        s(contraste_z,SiteCode,bs="fs") + s(SiteCode,bs="re"),
      family="binomial",data=df,method="REML")
}
l_md_congContrastes <- dlply(df_congContrastes,"pair",f_mdGAMMcongCont)
...
f_gam_logOR <- function(df,ssbs.type="tp"){
  l_md <- list()
  l_md[[1]] <- gam(logOR_value ~ 
                     s(k_z,bs = ssbs.type) + s(p_z,bs = ssbs.type) + ti(p_z,k_z) +
                     s(k_z, SiteCode, bs="fs",xt=list(bs=ssbs.type)) +
                     s(SiteCode,bs="re"),
                   data=df, method="REML")
  l_md[[2]] <- gam(logOR_value ~ 
                     s(contraste_z,bs = ssbs.type) + 
                     s(contraste_z, SiteCode, bs="fs",xt=list(bs=ssbs.type)) +
                     s(SiteCode,bs="re"),
                   data=df, method="REML")
