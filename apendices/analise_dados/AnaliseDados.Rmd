...
df_ad <- distinct(select(df_resultados,-(Smed:Smax))) |>
  arrange(p) |> 
...
md_frag_f.area <- gam(r3_frag_z ~ 
                        s(r3_area_z,bs="cr") + s(r3_area_z,SiteCode,bs="fs",xt=list(bs="cr")),
                      data=filter(df_md,logOR_pair == "cont.non_frag"))
```
...
df_aicctab <- AICctab(l_md.logOR,weights=TRUE) |> as.data.frame()
df_aicctab$modelo <- row.names(df_aicctab)
row.names(df_aicctab) <- NULL
df_dev.exp <- ldply(l_md.logOR,.fun = \(x) summary(x)$dev.expl)
names(df_dev.exp) <- c("modelo","dev.expl")
df_tabelaSelecao <- df_dev.exp |> 
...
```
```{r}
df_aicctab_tp <- AICctab(l_md.logOR_tp,weights=TRUE) |> as.data.frame()
df_aicctab_tp$modelo <- row.names(df_aicctab_tp)
row.names(df_aicctab_tp) <- NULL
df_dev.exp_tp <- ldply(l_md.logOR_tp,.fun = \(x) summary(x)$dev.expl)
names(df_dev.exp_tp) <- c("modelo","dev.expl")
df_tabelaSelecao_tp <- df_dev.exp_tp |> 
  inner_join(y=df_aicctab_tp,"modelo") |> 
  arrange(dAICc) |> 
  select(modelo,dAICc:weight,dev.expl)
df_tabelaSelecao_tp
```
...
```{r logOR GAMM}
md_logOR <- l_md.logOR_tp[[df_tabelaSelecao_tp$modelo[1]]] 
k.check(md_logOR)
...
f_predSitio1d <- \(md,
  ...
  df_plot |> 
    ggplot(aes(x=r3_frag_z,y=logOR)) +
