...
f_randomEffects <- function(disp_var,df=df_md,poly=polyK){
  if(disp_var == "k"){
    l_md <- list()
    l_md[[1]] <- glmer(cbind(nCongKS,100-nCongKS) ~ poly(p_z,polyK,raw=T) * k + (1|SiteCode),
          data=df, family = "binomial",
          control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e9)))
    l_md[[2]] <- glmer(cbind(nCongKS,100-nCongKS) ~ poly(p_z,polyK,raw=T) * poly(k_cont_z,polyK,raw=T) + (1|SiteCode),
          data=df, family = "binomial",
          control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e9)))
    l_md[[3]] <- glmer(cbind(nCongKS,100-nCongKS) ~ 
                         poly(p_z,polyK,raw=T) * poly(k_cont_z,polyK,raw=T) + (poly(k_cont_z,polyK,raw=T)|SiteCode),
          data=df, family = "binomial",
          control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e9)))
...
########
save(m_MNEE,file="dados/Rdata/md_MNEE.Rdata")
