...
Rdata_path="./dados/Rdata/glmm_re_"
...
```{r construcao de modelos simulado e predito}
...
```{r}
n <- load("./dados/Rdata/glmm_l_md_nCong.Rdata",verbose = TRUE)
l_md <- eval(as.name(n))
rm(list=n)# 
# f_bestfit <- \(md_i){
#   lmd_allfit <- allFit(md_i,maxfun = 1e5, parallel = 'multicore', ncpus = 2)
#   lmd_allfit_diag <- lmd_allfit[sapply(lmd_allfit, is, "merMod")]
#   n_lmd <- ldply(lmd_allfit_diag,f_warning,.id="glmer") %>% 
#     filter(V1!="OK") %>% pull("glmer")
# }
f_warning <- function(x) x@optinfo$conv$lme4$messages %>% 
  paste(.,collapse = " , ") %>% sub("$","NA",.)
v_convmd <- ldply(l_md,f_warning,.id="glmer") %>%
    filter(V1=="NA") %>% pull("glmer")
l_md <- l_md[v_convmd]
names(l_md)
df_ts <- AICctab(l_md,weights=TRUE) %>% as.data.frame()
l_md[[rownames(df_ts)[1]]]
p_plot <- simulateResiduals(l_md[[rownames(df_ts)[1]]],n = 500)
plot(p_plot)
testOutliers(l_md[[rownames(df_ts)[1]]])
plot(l_md[[rownames(df_ts)[1]]])
