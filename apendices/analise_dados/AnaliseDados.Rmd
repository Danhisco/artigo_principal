...
```{r selecao estrutura aleatoria, echo=T,eval=F}
...
f_randomEffects <- function(disp_var,df=df_md){
  if(disp_var == "k"){
    ...
    l_md[[2]] <- glmer(cbind(nCongKS,100-nCongKS) ~ poly(p_z,2) * poly(k_cont_z,2) + (1|SiteCode),
          data=df, family = "binomial",
          control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e9)))
    l_md[[3]] <- glmer(cbind(nCongKS,100-nCongKS) ~ poly(p_z,2) * poly(k_cont_z,2) + (poly(k_cont_z,2)|SiteCode),
          data=df, family = "binomial",
          control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e9)))
    names(l_md) <- c("k + 1|Site","k_cont^2 + 1|Site","k_cont^2 + k_cont^2|Site")
  } else if(disp_var == "d"){
    l_md <- list()
    l_md[[1]] <- glmer(cbind(nCongKS,100-nCongKS) ~ poly(p_z,2) * poly(d_z,2) + (1|SiteCode),
          data=df, family = "binomial",
          control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e9)))
    l_md[[2]] <- glmer(cbind(nCongKS,100-nCongKS) ~ poly(p_z,2) * poly(d_z,2) + (poly(d_z,2)|SiteCode),
          data=df, family = "binomial",
          control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e9)))
    names(l_md) <- c("d^2 + 1|Site", "d^2 + d^2|Site")
  } else if(disp_var == "d.Lplot"){
    l_md <- list()
    l_md[[1]] <- glmer(cbind(nCongKS,100-nCongKS) ~ poly(p_z,2) * poly(d.Lplot_z,2) + (1|SiteCode),
          data=df, family = "binomial",
          control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e9)))
    l_md[[2]] <- glmer(cbind(nCongKS,100-nCongKS) ~ poly(p_z,2) * poly(d.Lplot_z,2) + (poly(d.Lplot_z,2)|SiteCode),
          data=df, family = "binomial",
          control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e9)))
    names(l_md) <- c("d.Lplot^2 + 1|Site", "d.Lplot^2 + d.Lplot^2|Site")
  } else if(disp_var == "log(d)"){
    l_md <- list()
    l_md[[1]] <- glmer(cbind(nCongKS,100-nCongKS) ~ poly(p_z,2) * poly(log_d_z,2) + (1|SiteCode),
          data=df, family = "binomial",
          control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e9)))
    l_md[[2]] <- glmer(cbind(nCongKS,100-nCongKS) ~ poly(p_z,2) * poly(log_d_z,2) + (poly(log_d_z,2)|SiteCode),
          data=df, family = "binomial",
          control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e9)))
    names(l_md) <- c("log(d)^2 + 1|Site", "log(d)^2 + log(d)^2|Site")
  } else if(disp_var == "log(d.Lplot)"){
    l_md <- list()
    l_md[[1]] <- glmer(cbind(nCongKS,100-nCongKS) ~ poly(p_z,2) * poly(log_d.Lplot_z,2) + (1|SiteCode),
          data=df, family = "binomial",
          control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e9)))
    l_md[[2]] <- glmer(cbind(nCongKS,100-nCongKS) ~ poly(p_z,2) * poly(log_d.Lplot_z,2) + (poly(log_d.Lplot_z,2)|SiteCode),
          data=df, family = "binomial",
          control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e9)))
...
l_md <- unlist(unname(l_md),recursive = F)
save(l_md,file = "dados/Rdata/l_md_RE_MNEE.Rdata")
```
...
```{r dHARMa residuos md_MNEE_RE}
p_plot <- DHARMa::simulateResiduals(l_md[[3]],n=1000)
plot(p_plot)
md_MNEE <- glmer(cbind(nCongKS,100-nCongKS) ~ poly(p_z,2) * poly(k_cont_z,2) + (poly(k_cont_z,2)|SiteCode),
          data=df_ad, family = "binomial",
          control=glmerControl(optimizer="bobyqa",optCtrl=list(maxfun=2e9)))
MuMIn::r.squaredGLMM(md_MNEE)
```

```{r AICctab md_MNEE_RE}
