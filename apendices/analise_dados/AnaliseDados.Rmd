...
```{r}
md_PrCong_land.hyp_k <- gam(cbind(nCong,100-nCong) ~ p_z * k_factor * land_hyp + 
                              s(SiteCode,land_hyp,bs='re') +
                              s(SiteCode,bs='re'),
                              family = "binomial",data=mutate(df_md,
...
df_ip_prcong_landhpyp_k <- f_calcPI(gamm = md_PrCong_land.hyp_k,prcong_glmer = TRUE)
