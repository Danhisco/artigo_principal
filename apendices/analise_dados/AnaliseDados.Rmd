...
f_newdata_ad4 <- \(l_md_newdata,quantiles=c(0.05,0.95)){
  ...
  f_newdf <- \(d){
    d <- d %>% 
      ...
      select(-diff)
    f_seq <- \(X){
      areaU_z <- do.call("seq", as.list(c(from=X$Q05,to=X$Q95,length.out=X$maxsize)))
      cbind(select(X,k_z,SiteCode),areaU_z)
      }
    adply(d,1,f_seq) %>% 
      select(k_z:areaU_z)}
  # 1a camada areaU_z ~ f(k)
  md <- l_md_newdata[[names(l_md_newdata) %>% str_which(.,"areaU ~")]]
  df <- data.frame(k_z = do.call("seq", as.list(c(range(md$model$k_z),length.out=150))),
                   SiteCode = "SPigua1")
  vname="areaU_z"
  df_newdata <- ddply(data.frame(q = c("Q05","Q95"),quantiles),
                      "q",\(d) qgam::qdo(md,d$quantiles,f_predict)) %>% 
    pivot_wider(names_from="q",values_from="areaU_z") %>% 
    f_newdf()
  # 2a camada: frag ~ f(area,k)
  md <- l_md_newdata[[names(l_md_newdata) %>% str_which(.,"fragU ~")]]
  vname="fragU_z"
