...
f_formulaGAMM <- \(string){
  if(string == "f(p,k)"){
    as.formula(paste("cbind(nCong,100-nCong)",
                     "~ s(p_z,bs='cr') + s(k_z,bs='cr') + s(k_z,Site,bs = 'fs') + s(Site, bs = 're')"))
...
l_md <- vector("list",5)
names(l_md) <- c("f(p,k)", # p: 4x4km dependente , k: variável que controla a maior escala da AGC
                 "f(areaU,fragU)","f(areaSAD,fragSAD)", # var padronistas
                 "f(contU)","f(contSAD)") # var não padronistas
