...
df_md <- df_md |> 
  ...
  rename(contU_z = contraste_z, contSAD_z = contrasteSAD_z,pair=logOR_pair)
...
l_f[[2]] <- "logOR_value ~ s(contU_z,bs='cr') + s(p_z,bs='cr') + ti(k_z,p_z) + s(k_z, SiteCode, bs='fs',xt=list(bs='cr')) +s(SiteCode,bs='re')"

f_gam <- \(f){
  gam(formula = f,data = filter(df_md,pair=="cont.non_frag"),method = "REML")
}
md_teste <- f_gam(teste)
