...
df_obs |> 
  ...
  annotate("rect",xmin = -1.05,xmax=0.855,ymin=13,ymax=16.9) +
  annotate("text",x = -0.1,y = 15,
           label=paste0("dev. exp. = ",round(v_devexp*100,digits = 1),"%*"),color="white") +
  labs(subtitle = "horizontal lines = quantile(x, probs=c(0.95,0.99))",
       caption = "*full model") +
...
f_mdGAMM <- \(df){
  gam(cbind(nCong,100-nCong) ~ 
        s(contraste_z) + 
        s(contraste_z,SiteCode,bs="fs") + s(SiteCode,bs="re"),
      family="binomial",data=df,method="REML")
}
l_md_congContrastes <- dlply(df_congContrastes,"pair",f_mdGAMM)
