...
```{r}
gamm_MNEE <- l_md_GAMM[[row.names(df_aicctab)[2]]]
k.check(gamm_MNEE)
gratia::appraise(gamm_MNEE)
# summary(gamm_MNEE)
gratia::draw(gamm_MNEE)
...
```{r}
(gamm_MNEE <- l_md_GAMM[[row.names(df_aicctab)[3]]])
k.check(gamm_MNEE)
gratia::appraise(gamm_MNEE)
# summary(gamm_MNEE)
gratia::draw(gamm_MNEE)
...
```{r}
(gamm_MNEE <- l_md_GAMM[[row.names(df_aicctab)[4]]])
k.check(gamm_MNEE)
gratia::appraise(gamm_MNEE)
# summary(gamm_MNEE)
gratia::draw(gamm_MNEE)
...
O modelo tem dificuldade de distinguir os efeitos parciais do modelo 

#### predição

__Como construir o novo conjunto de dados?__

Resumo da seleção de modelos

__Qual a melhor forma de descrever as preditoras em função de si mesmas?__

```{r melhor forma de descrever as preditoras}
df_md <- df_md0 |> filter(k=="0.99")
l_md <- list()
l_md[[1]] <- gam(log_Ntotal_z ~ s(p_z,bs="cr"),data = df_md)
l_md[[2]] <- gam(log_S_obs_z ~ s(p_z,bs="cr"),data = df_md)
l_md[[3]] <- gam(log_Ntotal_z ~ s(log_S_obs_z,bs="cr"),data = df_md)
l_md[[4]] <- gam(log_S_obs_z ~ s(log_Ntotal_z,bs="cr"),data = df_md)
l_md[[5]] <- gam(log_Ntotal_z ~ s(p_z,bs="cr") + s(log_S_obs_z,bs="cr"),data = df_md)
l_md[[6]] <- gam(log_S_obs_z ~ s(p_z,bs="cr") + s(log_Ntotal_z,bs="cr"),data = df_md)
...
A seleção de modelos indica que a melhor forma de reconstruir os dados é:
i) predizer novo conjunto de dados de S a partir de p; e depois
ii) predizer novo conjunto de dados de N a partir de S.
