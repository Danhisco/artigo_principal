...
df_ad <- inner_join(x=select(df_resultMNEE,-(Ssd:Smax)),
  ...
  mutate(d.Lplot = d / sqrt(10000*effort_ha),
         ...
         across(c(p,Ntotal:log_d.Lplot),f_z,.names = "{.col}_z")) |> 
...
f_randomEffects <- function(disp_var,df=df_md){
  if(disp_var == "k"){
    ...
    l_md[[1]] <- glmer(cbind(nCongKS,100-nCongKS) ~ poly(p_z,2) * poly(d_z,2) + (1|SiteCode),
          data=df, family = "binomial",
          control=glmerControl(optimizer="Nelder_Mead",optCtrl=list(maxfun=2e9)))
    l_md[[2]] <- update(l_md[[1]], . ~ ., -(1|SiteCode) + (poly(d_z,2)|SiteCode))
    ...
    l_md[[1]] <- glmer(cbind(nCongKS,100-nCongKS) ~ poly(p_z,2) * poly(d.Lplot_z,2) + (1|SiteCode),
          data=df, family = "binomial",
          control=glmerControl(optimizer="Nelder_Mead",optCtrl=list(maxfun=2e9)))
    l_md[[2]] <- update(l_md[[1]], . ~ ., -(1|SiteCode) + (poly(d.Lplot_z,2)|SiteCode))
    ...
    l_md[[1]] <- glmer(cbind(nCongKS,100-nCongKS) ~ poly(p_z,2) * poly(log_d_z,2) + (1|SiteCode),
          data=df, family = "binomial",
          control=glmerControl(optimizer="Nelder_Mead",optCtrl=list(maxfun=2e9)))
    l_md[[2]] <- update(l_md[[1]], . ~ ., -(1|SiteCode) + (poly(log_d_z,2)|SiteCode))
    ...
    l_md[[1]] <- glmer(cbind(nCongKS,100-nCongKS) ~ poly(p_z,2) * poly(log_d.Lplot_z,2) + (1|SiteCode),
          data=df, family = "binomial",
          control=glmerControl(optimizer="Nelder_Mead",optCtrl=list(maxfun=2e9)))
    l_md[[2]] <- update(l_md[[1]], . ~ ., -(1|SiteCode) + (poly(log_d.Lplot_z,2)|SiteCode))
...
```{r}
teste <- allFit(l_md[[1]])
teste.OK <- sapply(teste, is, "merMod")
lapply(teste[teste.OK],function(x) x@optinfo$conv$lme4$messages)
```
