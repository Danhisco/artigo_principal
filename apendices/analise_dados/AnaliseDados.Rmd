...
df_ad <- inner_join(y=distinct(select(df_resultados,-(Smed:Smax))),
                    x=distinct(select(df_dados_disponiveis,SiteCode,forest_succession)),
                    by="SiteCode") |>
  arrange(p) |> 
  mutate(across(p:k,f_z,.names = "{.col}_z"))
...
df_md <- df_ad |> 
  ...
  mutate(SiteCode = factor(SiteCode),
         across(starts_with("efeito"),~sign(.x) * abs(.x)^(1/3),.names = "cubeR_{.col}")) |> 
  select(-land_type,-propCong,-logOR_pair,-p,-k)
  # df_newdata
...
```{r GE efeito área e as possíveis 4 preditoras}
df_contrastes |> 
  ...
  labs(title="p e k parecem ser suficientes para descrever os contrastes") +
  facet_grid(pred_class~resp_class,scales="free")
```

__Figura X__. Gráficos exploratórios das possíveis preditoras para descrever o contraste de área


__Janela de código X__ GAMM usado para ajustar os contrastes 

```{r GAMM contrastes, echo=T}
f_gam <- function(df){
  gam(value ~
...
# logOR contemporâneo / sem frag

### GE

```{r GE logOR contem sem fra}
df_md <- df_resultados |> filter(logOR_pair == "cont.non_frag") |> 
  inner_join(x=df_contrastes |> 
               select(SiteCode:k,efeito_area:efeito_frag),
             by=c("SiteCode","k")) |> 
  mutate(SiteCode = factor(SiteCode),
         across(c(p:k,efeito_area:efeito_frag),f_z,.names = "{.col}_z")) |> 
  select(SiteCode,contains("_z"),logOR_value) |> 
  rename(logOR = logOR_value)
df_md |> 
  pivot_longer(p_z:k_z) |> 
