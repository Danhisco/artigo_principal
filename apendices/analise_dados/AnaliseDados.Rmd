...
l_md[[2]] <- gam(cbind(nCongKS,100-nCongKS)  ~ 
                   ...
                   s(SiteCode,bs="re"),
                 data = df_md, family="binomial", method = "REML")
names(l_md) <- c("s(p,k) + ti(p,k)","s(p,k)")
```
...
O modelo mais palusível, com 84% do peso de evidência, possui um spline para cada variável. 
...
- intro geral: diff razoável SAD obs e SAD predita

#### A forma da SAD predita 

DAG inner cicle: Pr(Cong) ~ N, S, p, k
...
#### Processo gerador de inventários florestais

Devido aos viéses de pesquisa relacionada com o esforço amostral, podemos esperar que N, Sobs e p possam covariar. As variáveis que podem conter informação sobre esforço amostral podem ser: proximidade com centros de pesquisa ou urbanos (Prox), diversidade local esperada (e.g. em termos de riqueza esperada - Sesp), e grau de perturbação esperado (PERTesp). Todas essas são variáveis latentes no momento que podem ser explicadas por outras variáveis geográficas latentes (e.g. qualidade do solo e perfil geográfico). As variáveis latentes devem conter informação sobre p, N e Sobs via esforço amostral ou diretamente. O esforço amostral deve conter informação sobre N e Sobs, e N sobre Sobs. Sesp deve conter informação sobre Sobs. PERTesp pode conter informação sobre p e Sesp. Pressupus que não há influência de Sesp em PERTesp (mas esse pressuposto pode ser relaxado sem mudanças nas conclusões do DAG).

DAG outter cirvle: S ~, N ~, p ~ latent variables
