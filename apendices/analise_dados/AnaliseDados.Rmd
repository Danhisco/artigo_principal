...
l_f[[2]] <- "~ k_z + land_hyp + (k_z + land_hyp|SiteCode)"
l_f[[3]] <- "~ land_hyp + (land_hyp|SiteCode)"
l_f[[4]] <- "~ 1 + (1|SiteCode)"
#
# allFit(l_nRef__modeloCheio[v_glmerUpdate][[i]],maxfun = 1e5, parallel = 'multicore', ncpus = 3)
#
names(l_f) <- laply(l_f,\(s) trimws(str_extract(s, "~(.*?)(?=\\+)")))
f_lmd <- \(lf,Rdata_path="./dados/Rdata/glmm_"){
...
## versão ad4
l_f <- list()
l_f[[1]] <- "~ land_hyp + s(land_hyp,SiteCode,bs='re')"
l_f[[2]] <- gsub("\\+",
                 "\\+ s\\(k_z,by\\=land_hyp,bs\\='cr',k\\=1\\) \\+ s\\(k_z,by\\=land_hyp,SiteCode,bs\\='fs',k\\=1\\) \\+",
                 l_f[[1]])
l0 <- as.list(paste0("land_hyp + s(",
                     names(select(df_md,p_z,ends_with("U_z"))),
                     ",by=land_hyp,bs='cr',k=1) + "))
l0[length(l0)+1] <- "land_hyp + s(areaU_z,by=land_hyp,bs='cr',k=1) + s(fragU_z,by=land_hyp,bs='cr',k=1) + "
l0 <- sapply(l0, \(s) str_replace(l_f[[2]],"land_hyp\\s\\+\\s",s))
l_f <- c(l_f,l0)
l0 <- c("f(land. hyp.|Site)","f(k * land. hyp.|Site)")
names(l_f) <- c(l0,
                sapply(paste0("(",gsub("_z","",names(select(df_md,p_z,ends_with("U_z")))),", k") %>% 
                         gsub("U","",.), 
                       \(s) sub("\\(k",s,l0[2])),
                "f(area, frag, k * land. hyp.|Site)") %>% unname()
df_formulas <- data.frame(nome = names(l_f),
                          formula = unlist(l_f))  
