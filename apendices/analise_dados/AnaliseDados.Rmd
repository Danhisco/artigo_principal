...
df_md <- df_ad |> 
  ...
  select(-land_type,-propCong,-c(p:k),-c(efeito_area:cubeR_efeito_conf)) |> 
  rename(logOR = logOR_value,nSADcong=nCongKS) |> 
  rename_with(.fn = \(x) str_replace(x,pattern = "cubeR_efeito_",replacement = "r3_"),
              .cols = starts_with("cubeR"))
  # df_newdata
...
df_md |> 
  filter(logOR_pair == "cont.non_frag") |> 
  pivot_longer(c(p_z:efeito_frag_z,cubeR_efeito_area_z:cubeR_efeito_frag_z)) |> 
...
f_gam_logOR <- function(df){
  ...
  l_md[[1]] <- gam(logOR ~ 
                   ...
                   data=df, method="REML")
  l_md[[2]] <- gam(logOR ~ 
                     te(cubeR_efeito_area_z,cubeR_efeito_frag_z,k=15) +
                     t2(cubeR_efeito_area_z,cubeR_efeito_frag_z,SiteCode,k=15,bs=c("cr","cr","re"),full=TRUE),
                   data=df, method="REML")
