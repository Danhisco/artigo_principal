...
df_md <- df_ad |> 
  inner_join(df_contrastes |> 
               ...
               mutate(across(contains("efeito"),f_z,.names="{.col}_z")) |> 
...
CPF:
1) protocolo de validação: 5 elementos - k.check, summary, appraise, draw, e png do predito e observado por sítio
...
if(gamm$family$family=="binomial"){
  df_obs <- gamm$model |> select(-1) |> #@ esse trecho é válido apenas para modelos binomiais
...
df_newpred <- data.frame(x = seq(min(df_obs[,x_var]),
                                 max(df_obs[,x_var]),
                                 length.out=length_out_x)) |> 
  mutate(SiteCode = v_site)
...
df_obs |> 
  ...
  annotate("text",x = 0,y = 15,
           label=paste0("dev. exp. = ",round(v_devexp*100,digits = 1),"%*"),color="white") +
  labs(x="z(contraste)",y="congruência",
       title="observado e predito pelo modelo completo e sem a variação entre sítios",
       subtitle = "horizontal lines = quantile(x, probs=c(0.95,0.99))",
       caption = "*full model") +
  geom_vline(xintercept = quantile(df$area_z,probs = c(0.95,0.99)),linetype="dashed",alpha=0.3) +
