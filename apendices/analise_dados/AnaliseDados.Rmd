...
Diagnósticos Congruência entre Contrastes

1) Contenporâneo e Idealizado

Os smoothers são significativos segundo o número de funções base adequadas e quanto a comparação com um intercepto. 
O deviance explainde é de 85.6%. 
Pode existir erros sistemáticos na deviance (resíduos), apesar do predito quanto observado estarem em congruência no padrão geral.
Quanto maior o contraste estimado maior a variabilidade estimada do efeito parcial, com o efeito médio tendendo à zero e os extremos variando de 10 até -10. 

2) Contemporaneo e Sem Fragmentação

Os smoothers são significativos segundo as funções do mgcv.
O dev. exp. é 76.8%
Os resíduos parecem ter um viés sistmático, mas o predito observado parecem convergir.
Os efeitos parciais do contraste também apresentam padrão de aumentar em variabilidade com o aumento do contraste, mas eles tendem a reduzir com nos extremos do gradiente da preditora.

3) Sem Fragmentação e Idealizado

Os smoothers são significativos segundo as funções do mgcv.
O dev. exp. é 54%.
Os resíduos não estão muito bem distribuídos.
A variabilidade dos efeitos parciais aumenta com o valor do contraste

```{r figura final GAMM CongContrastes}
...
#
l_md <- list()
l_md[[1]] <- gam(diffS ~ land_hyp + s(p,by=land_hyp) + s(SiteCode,bs="re"),
                 data=df_md,method="REML")
l_md[[2]] <- gam(diffS ~ land_hyp + s(SiteCode,bs="re"),
                 data=df_md,method="REML")
l_md[[3]] <- gam(diffS ~ 1 + s(SiteCode,bs="re"),
                 data=df_md,method="REML")
names(l_md) <- c("~ f(p,land. hyp)","~ f(land. hyp)","~ 1")
...
f_gam_logOR <- function(df){
  l_md <- list()
  l_md[[1]] <- gam(logOR_value ~ 
                     s(k_z,bs = "cr") + s(p_z,bs = "cr") + ti(p_z,k_z) +
                     s(k_z, SiteCode, bs="fs",xt=list(bs="cr")) +
                     s(SiteCode,bs="re"),
                   data=df, method="REML")
  l_md[[2]] <- gam(logOR_value ~ 
                     s(contraste_z,bs = "cr") + 
                     s(contraste_z, SiteCode, bs="fs",xt=list(bs="cr")) +
                     s(SiteCode,bs="re"),
                   data=df, method="REML")
...
l_md.logOR <- dlply(df_md,"logOR_pair",f_gam_logOR)
...
```{r}
# validação
for(pair in names(l_md_congContrastes)){
  md <- l_md_congContrastes[[pair]]
  print(pair)
  print(k.check(md))
  print(summary(md))
...
# dados
f_wider <- \(df){
  df_y <- df |> filter(land_hyp == "frag") |> select(SiteCode:p_z)
  df_x <- df |> select(SiteCode,k_z,land_hyp:last_col()) |> 
    pivot_wider(names_from=land_hyp,values_from = c(U_z,SAD_z),names_glue = "{land_hyp}.{.value}")
  inner_join(df_y,df_x)
}
df_md1 <- df_md |> 
  select(SiteCode,nCong,k_z,p_z,land_hyp,contrasteSAD_z,contraste_z) |> 
  rename(U_z=contraste_z,
         SAD_z=contrasteSAD_z) |> 
  mutate(land_hyp = case_when(land_hyp == "cont" ~ "frag",
                              land_hyp == "ideal" ~ "cont",
                              land_hyp == "non_frag" ~ "area",
                              TRUE ~ NA)) |> 
  f_wider()
# GE
df_md1 |> 
  pivot_longer(-(SiteCode:nCong)) |> 
  ggplot(aes(x=value,y=nCong)) +
