...
df_contrastes |> 
  pivot_longer(cols = c(k:p,log.N_z:log.S_z)) |> 
  mutate(name = factor(name,levels=c("p","k","log.N_z","log.S_z"))) |> 
  ggplot(aes(x=value,y=efeito_area)) +
  geom_point() +
  geom_quantile(method="rqss",quantiles = c(0.05,0.5,0.95),lambda=0.1,color="darkred") +
  labs(title="p e k parecem ser suficientes para descrever o contraste de área") +
...
md <- l_md.contrastes[["efeito_area"]]
print("k.check:")
k.check(md)
print("summary:")
summary(md)
...
ggsave("apendices/analise_dados/figuras/contraste_area_site_kGAMM.png",
       width = 7,
       height = 30)
