...
Para descrever a probabilidade de boa congruência entre SAD predita e empírica utilizei um GAMM binomial. Há uma estrutura comum entre todos os modelos candidatos: um spline individual geral (estrutura fixa) para `k_z`, um spline para `k_z` para cada sítio de amostragem, e um intercepto por sítio de amostragem - para cada um desses 3 termos há uma cópia por tipo de paisagem hipotética: contemporânea, sem fragmentação, e idealizada. Entre os modelos candidatos há diferentes possibilidades de covariáveis de k que também são copiados para cada tipo de paisagem (sensu Pedersen et al 2019). Cada um dos 3 tipos de contrastes (area per se, frag. per se., e contemporaneidade per se) e 2 tipos de variáveis (taxa U e logito da congruência entre SADs preditas) e a proporção de cobertura vegetal (p) tiveram um modelo com um spline indivíduo geral (estrutura fixa). Também considerei modelos que combinavam os contrastes de área per se e frag. per se com um spline individual geral (estrutura fixa) para cada contraste. Totalizando 10 modelos. 
...
Para descrever o contraste de área é adequado considera os splines individuais de k, fixo e aleatórios (summario do modelo). Para descrever o contraste de fragmentação per se o mais plausível é considerar o spline fixo do contraste de área em adição aos splines de k, fixos e aleatórios (tabela de seleção). 

Para construir o conjunto de dados usado na predição, primeiro descrevi como os quantis de 5% e 95% variam em função das outras preditoras. Comparei os modelos usando a descrição do valor médio de cada preditora como variável resposta. Então avaliei como os quantis de 5% e 95% são descritos segundo a relação proposta pela formula do modelo mais plausível. Esses dois quantis foram usados como valores extremos para criar uma sequência de valores intermediários.