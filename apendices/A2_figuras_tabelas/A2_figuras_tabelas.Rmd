...
df_mnames <- df_tab |> group_by(pair) |> slice_head() |> select(pair,modelo)
f_a_ply <- \(df){
  print(paste0("Contraste: ",df$pair))
  f_validaGAMM(md_name = df$modelo,l_md = l_md_congContrastes[[df$pair]],size=0)
}
a_ply(df_mnames,1,f_a_ply)
```
