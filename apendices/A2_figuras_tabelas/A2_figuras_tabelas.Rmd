...
### Figura A2 3
```{r fig A23 ctror, eval=F,include=F}
...
```{r fig A2 4 ctror,eval=F,include=F}
l_p <- list()
l_p[[1]] <- df_contrastes |>
...
save(l_p,file="./dados/Rdata/figA24.Rdata")
```

```{r Figura A2 4 GE contrastes}
load("./dados/Rdata/figA24.Rdata")
...
```{r janela output 3 ctror,eval=F,include=F}
load("./dados/Rdata/l_md_congContrastes.Rdata")
df_tab <- read_csv("./dados/csv/ts_congCont.csv") %>% as.data.frame()
# df_tab
...
save(l_md_plot,file="./dados/Rdata/janelaA23.Rdata")
```

```{r Janela de output R console 3}
load("./dados/Rdata/janelaA23.Rdata")
...
```{r figA26 ctror,eval=F,include=F}
md_diffs <- lmer(diffS ~ land_hyp * p + (1|SiteCode),df_md)
p_dharma <- simulateResiduals(md_diffs,n = 1000)
save(p_dharma,md_diffs,file="./dados/Rdata/figA26.Rdata")
```

```{r graf exp diffS md + lik}
load("./dados/Rdata/figA26.Rdata")
plot(p_dharma)
```
...
### Pr(Cong)
```{r figA27 ctror,eval=F,include=F}
df_tab <- read_csv("./dados/csv/ts_PrCong_LandHyp_GAMMs.csv")
load("./dados/Rdata/l_md_PrCong.Rdata")
# df_tab
df_mdlik <- df_tab |> filter(dAICc == 0) |> select(land_hyp,modelo)
l_md_plot <- alply(df_mdlik,1,\(df) l_md_PrCong[[df$land_hyp]][[df$modelo]])
names(l_md_plot) <- df_mdlik$land_hyp
save(l_md_plot,file="./dados/Rdata/figA27.Rdata")
```

```{r figA27 - janela de código da validacao dos submodelos}
load("./dados/Rdata/figA27.Rdata")
l_ply(names(l_md_plot),\(string) f_validaGAMM(md_name = string,l_md=l_md_plot))
```

#### Detalhes da análise dos dados

__Contexto__

Para descrever a probabilidade de boa congruência entre SAD predita e empírica utilizei GAMMs binomial. Há uma estrutura comum entre todos os modelos candidatos: um spline individual geral (estrutura fixa) para `k_z`, um spline para `k_z` para cada sítio de amostragem, e um intercepto por sítio de amostragem - para cada um desses 3 termos há uma cópia por tipo de paisagem hipotética: contemporânea, sem fragmentação, e idealizada. Entre os modelos candidatos há diferentes possibilidades de covariáveis de k que também são copiados para cada tipo de paisagem (sensu Pedersen et al 2019). Cada um dos 3 tipos de contrastes (area per se, frag. per se., e contemporaneidade per se) e 2 tipos de variáveis (taxa U e logito da congruência entre SADs preditas) e a proporção de cobertura vegetal (p) tiveram um modelo com um spline indivíduo geral (estrutura fixa). Também considerei modelos que combinavam os contrastes de área per se e frag. per se com um spline individual geral (estrutura fixa) para cada contraste. Totalizando 10 modelos. 
...
O único modelo plausível foi aquele com splines na estrutura fixa para k e os contrastes na taxa U de área e frag.
...
Esse modelo apresenta ajuste razoável, com boas estimativas dos efeitos parciais médio para todos os splines individuais na estrutura aleatória, exceto para o spline de k em paisagens contemporâneas. Portanto, a interpretação dos resultados usando um intervalo de predição a posteriori será feita com esse modelo mais plausível.




Os contrastes de U (area e frag.) apresentam elevada assimetria, são pequenos em k elevado e podem ser elevados em k pequenos (Figura 1).
Entendo que é adequado criar um conjunto de dados para interpretação do intervalo de predição a posteriori que explore combinações representativas da relação entre preditoras (k, area e frag.).
Busquei descrever como o intervalo quantílico varia com função de possíveis relações entre as 3 preditoras. A variável k pode ser interpretada como geradora das outras duas preditoras, pois se caso k tender a 100%, então não há influência da paisagem ao redor na dinâmica ecológica local/o que permite a influência da paisagem ao redor na dinâmica local é que a limitação de dispersão não é absoluta. Os dois programas de pesquisa da ecológia de paisagens entendem que o efeito de área per se não pode ser gerado pelo efeito de fragmentação per se, ou seja, o contraste de área pode ser uma preditora do contraste de fragmentação per se - o contrário, não. O programa de pesquisa padronista, pressupõe que  os efeitos de área e fragmentação per se, são ortogonais e, portanto, apenas k deve ser uma preditora adequada para descrever o contraste de fragmentação per se (Fahrig 2018/2019, ...). Já o programa de pesquisa processista pressupõe que existem efeitos indiretos da perda de área de habitat associadas aos efeitos de fragmentação per se (Putker et al. 2020), e portanto tanto k quanto o contraste de área podem ser preditoras adequadas para descrever o contraste de fragmentação per se. 
...
Avaliei como os quantis de 5% e 95% são descritos segundo a relação proposta pela formula do modelo mais plausível. Esses dois quantis foram usados como valores extremos para criar uma sequência de valores intermediários. Esse conjunto de dados será usado para construir dois tipos de intervalo de predição a posteriorido logito. O primeiro será comparando o predito pelo modelo mais plausível e o observado, agrupando para um mesmo valor de k e tipo de paisagem hipotética a variabilidade predita associada aos contrastes. No segundo, conjunto de dados irei filtrar os 20 valores de k simulados inicialmente, e então explorar a relação entre os contrastes de área e frag. per se e o logito de cada tipo de paisagem hipotética.
