...
```{r caption 2}
cap <- "Paisagens contemporâneas aos eventos de amostragem da SAD na área amostrada. Mapas de cobertura da coleção 6 do mapbiomas."
figHeight=28
figWidth=40
```
...
# 
# 
# df_p <- df_tif_txt %>% inner_join(df_plot %>% select(SiteCode,p) %>% distinct())
# f_plot <- \(df){
#   m <- read_table(df$txt.path) |> as.matrix()
#   image(m,
#         main=paste0(df$SiteCode,", p=",round(df$p,3)),
#         axes=FALSE,
#         zlim=c(0,2),
#         col=terrain.colors(12,rev = TRUE))
# }
# par(mfrow=c(11,10),
#     mar = c(mar=c(0.2,0.2,2.2,0.2)))
# a_ply(df_p,1,f_plot)
```
