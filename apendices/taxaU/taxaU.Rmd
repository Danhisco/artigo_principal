...
df_auditoria <- df_contrastes |> 
  # mutate(referencia_efeito = case_when(SiteCode %in% v_sitios_vis.igual ~ "p >= 0.93",
  #                                      p ==1 ~ "p = 1",
  #                                      TRUE ~ "p < 0.91"))
...
f_summarise <- \(df) adply(df,1,data.frame())
...
df_plot <- df_contrastes |> select(-starts_with("cubeR"),-efeito_conf) |>
  ...
  arrange(p) |> 
  mutate(label = paste0(SiteCode,", p=",round(p,2),"S=",S_obs,"N=",Ntotal))
df_plot$label <- factor(df_plot$label,levels=unique(df_plot$label))
df_plot |> 
