...
v_sitios_vis.igual <- df_contrastes |> 
  filter(p >= 0.77 & p < 1 &  
           !(SiteCode %in% 
               c("SPcana1","PRanto7","PRanto10","PRanto8","SPpeic1","PRanto5","PEbrejo","RSrioz1",""))) |> 
  pull(SiteCode) |> unique()
v_sitios_vis.igual <- c(v_sitios_vis.igual,
                        c("BAuruc","BAlenc4"))
...
df_auditoria <- df_contrastes |> 
  mutate(referencia_efeito = case_when(SiteCode %in% v_sitios_vis.igual ~ "visualmente igual",
                                       p ==1 ~ "p=1",
...
df_auditoria |> 
  ...
  ggplot(aes(x=name,y=value)) +
    geom_boxplot() +
    geom_jitter() + 
  labs(x="",y="contraste entre simulações") +
    # scale_color_manual(values=c("darkred","darkblue","darkgreen"),
                     # labels=c("contemporâneo","idealizado","sem fragmentação")) +
  facet_wrap(~referencia_efeito,ncol=2)
```
