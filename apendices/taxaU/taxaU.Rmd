...
df_contrastes <- df_U |>  
  ...
  mutate(efeito_area = (Umed_non_frag - Umed_ideal) / Umed_ideal,
         ...
         across(starts_with("efeito"),~sign(.x) * abs(.x)^(1/5),.names = "cubeR_{.col}")) |> 
  select(SiteCode:k,efeito_area:cubeR_efeito_conf) |> 
...
#
df_auditoria <- df_contrastes |> 
  mutate(referencia_efeito = case_when(SiteCode %in% v_sitios_vis.igual ~ "p >= 0.92 e visualmente igual",
                                       p ==1 ~ "p=1",
                                       TRUE ~ "algum grau diferente"))
df_auditoria |> 
  pivot_longer(efeito_area:efeito_conf) |>
  mutate(name = factor(name,labels = c("efeito area","efeito contemp","efeito frag"))) |> 
  ggplot(aes(x=name,y=value)) +
  geom_hline(data = df_auditoria |> 
               group_by(referencia_efeito) |> 
               summarise(min = min(across(efeito_area:efeito_conf,min))),
             aes(yintercept = min,
                 color=referencia_efeito),alpha=0.5) +
  scale_x_discrete(limits=c("efeito area","efeito frag","efeito contemp")) +
  geom_boxplot() +
  geom_jitter() + 
  labs(x="",y="contraste na estimativa da taxa U",
       subtitle = "efeito area = (sem isolamento - idealizado) / idealizado  \nefeito frag = (contemp - sem isolamento) / idealizado  \nefeito contemp = (contemp - idealizado) / idealizado",
       color="menor valor :") +
  theme_bw() +
...
df_auditoria |> 
# df_contrastes |> 
  # ggplot(aes(x=cubeR_efeito_area,y=cubeR_efeito_frag,color=referencia_efeito)) +
  ggplot(aes(x=efeito_area,y=efeito_frag,color = p==1)) +
  # ggplot(aes(x=log_efeito_area,y=log_efeito_frag)) +
  geom_abline(intercept = 0,slope = 1) +
  geom_line(aes(group=SiteCode),alpha=0.3) +
  geom_point(alpha=0.5) +
  scale_color_manual(name="p = 1", values = setNames(c('darkgreen','darkgray'),c(T, F)))
df_contrastes |> 
