...
# objetos
# df_resultsMNEE
df_resultados <- read_csv("dados/csv/resultados_MN/MNEE/cont/df_resultados.csv")
# df_sim
...
# dados
df_Urep <- list.files(path = "dados/csv/taxaU", pattern = ".csv",recursive = T,full.names = T) |> as.list() |>
...
df_contrastes <- df_U |>  
  ...
  mutate(efeito_area = (Umed_non_frag - Umed_ideal) / Umed_ideal,
         ...
         across(contains("efeito"), ~ log(ifelse(.x<=0,v_min,.x)),.names = "log_{.col}")) |>
  select(SiteCode:k,efeito_area:log_efeito_conf) |> 
  inner_join(x=distinct(select(df_sim,SiteCode:S_obs,txt.path:p)),by="SiteCode")
write_csv(df_contrastes,file="dados/csv/taxaU/MNEE/df_taxaU.csv")
# df_ad
```

```{r}
f_aply <- \(v) min(v[v>=0]) 
df_contrastes |> select(efeito_area:efeito_conf) |> as.data.frame
...
v_sitios <- c("MGuberl6","MGuberl5","RScach4","")
