...
Proposta:
i) transformar U para escala logito
ii) fazer seleção de modelos usando LMER para avaliar quais os possíveis offsets: S_obs, razão A_J / A_M, A_J, DA
iii) avaliar a melhor estrutura do GAMM 
...
df_Ulogit <- df_U |> 
  drop_na() |> 
  mutate(Ulogit = log(Umed/(1-Umed)),
         Areas_ratio = effort_ha / 1600,
         DA = Ntotal / effort_ha,
         S.z = f_z(S_obs),
         DA.z = f_z(DA),
...
l_md <- list()
l_md[[1]] <- lmer(Ulogit ~ )
lmer_U <- lmer(log10_U ~ log10_Sobs + (1|SiteCode),data=df_U)
