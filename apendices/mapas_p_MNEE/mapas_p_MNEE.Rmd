...
for(i in 1:nrow(df)){
  ...
  image(m,main=paste(df$SiteCode[i],", p =",round(df$p[i],3)))
  abline(h=0.5)
  abline(v=0.5)
  # print(dim(m))
}  
...
source("source/mapa_p_MNEE.R")
registerDoMC(2)
a_ply(df_sitios_se[-(1:5),],1,f_landscape_for_MNEE,) # lado_km = 16.02
```
...
"RScach4", "RScach2", "PRipir", "MGmadre4", "MGlavr6", "MGlavr3"
  
```{r salvar paisagens com as parcelas corrigidas,echo=FALSE,eval=FALSE}
df_correcao <-  df_auditoria |> filter(SiteCode %in% c("RScach4", "RScach2", "PRipir", "MGmadre4", "MGlavr6"))
