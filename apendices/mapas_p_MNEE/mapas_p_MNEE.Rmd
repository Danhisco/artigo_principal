...
```{r}
df_selecaoSitios <- df_pEscalas |> filter(lado_km>=15.99 & lado_km<=16.02) |> arrange(p)
df <- df_selecaoSitios |> head(n=5)
for(i in 1:nrow(df)){
  m <- raster(df$tif.path[i]) |> raster::as.matrix()
