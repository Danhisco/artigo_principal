...
a_ply(df$txt.path,1,f_auditoria)
...
1a leva: "RScach4", "RScach2", "PRipir", "MGmadre4", "MGlavr6", "MGlavr3"
2a leva: "SPpesm6", "RSsetape", "PRvent", 


```{r salvar paisagens com as parcelas corrigidas,echo=FALSE,eval=FALSE}
df_correcao <-  df_auditoria |> filter(SiteCode %in% c("SPpesm6", "RSsetape", "PRvent"))
...
df_sorteio_sitios$txt.file <- gsub("_null.txt",".txt",df_sorteio_sitios$txt.file)
registerDoMC(2)
a_ply(unique(df_sorteio_sitios$txt.file),1,f_nullLand,.parallel = TRUE)
```
