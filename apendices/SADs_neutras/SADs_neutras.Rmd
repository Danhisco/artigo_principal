...
# df_contrastes
df_contrastes <- read_csv(file="dados/csv/taxaU/df_taxaU.csv")
# df_ad
df_logOR <- df_resultados |> 
  # filter(logOR_pair == "cont.non_frag") |> 
  select(SiteCode:k,logOR_pair:logOR_value) |>
  pivot_wider(names_from = logOR_pair, values_from = logOR_value) |> 
  inner_join(y=select(df_contrastes,SiteCode,k,log_efeito_area:log_efeito_conf),by=c("SiteCode","k"))
```
...
df_resultados <- df_repMNEE |> 
  ...
  summarise(nCongKS = sum(p.KS>0.05),
            ...
            Smax = max(S),
            .groups = "drop") |> 
  mutate(propCong = case_when(nCongKS == 100 ~ 0.99/100,
                              nCongKS == 0 ~ 0.01/100,
                              TRUE ~ nCongKS / 100)) |> 
  inner_join(x=df_p,by="SiteCode")
#
f_ddply <- \(df,
             pairs = list(c("cont","non_frag"),c("cont","ideal"),c("non_frag","ideal"))){
  f <- \(v_string){
    ...
    data.frame(logOR_pair = paste(v_string,collapse = "."),
               logOR_value = log( (p1/(1-p1)) / (p2/(1-p2)) ) )
  }
