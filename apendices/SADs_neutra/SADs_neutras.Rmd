...
```{r arrumar os csv pois não fiz com o df_U deste Rmd que contém menos colunas}
f_arrumar <- function(df){
  m_SADrep <- read_csv(df$SADrep.path)
  df_write <- m_SADrep |> select(-(effort_ha:S_obs),-(d:Umax))
  write_csv(x = df_write, file = df$SADrep.path)
}
registerDoMC(2)
a_ply(df_SADrep,1,f_arrumar,.parallel = T)
```
