...
```{r diagnostico non frag- prístino}
...
```{r tabelas dos modelos da 2a fase}
library(knitr)
df <- data.frame(
...
```{r ajuste dos modelos da 2a fase de comparações}
df_md <- df_logOR %>% select(-(pristine:denominator),-matches("(^k|^p)")) %>% 
  rename(logOR = itself)
f_gam <- \(dfi){
  l_md <- list()
