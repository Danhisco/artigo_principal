...
saveRDS(df_contrastes,file="dados/csv_SoE/df_contrastes_z_e_padraor.rds")
df_contrastes <- df_contrastes %>%
  select(SiteCode,k,k_z,area_logratio_z:frag.total_logratio_z) %>% 
...
# df_mdsumar:
#@ com o logito estimado pelo modelo sumário, contem os logitos empiricos
df_mdsumar <- readRDS(file="./dados/csv_SoE/df_mdsumar.rds")
df_logOR <- df_mdsumar %>% 
  rename(logitoKS = logito) %>% 
  select(-c(nSAD,forest_succession,lat,long)) %>% 
