...
```
```{r 2a parte - ajuste de GAMM para descrever cada contraste}
...
f_ajuste <- \(dfi,responses=c("itself","pristine","numerator","denominator")){
  df_md <- dfi %>% pivot_longer(cols=all_of(responses),names_to = "response_class",values_to = "response")
  dlply(df_md,"response_class",f_gamm)
}
f_gamm <- \(dfmd){
  if(all(is.na(dfmd$response))) return(NA)
  gam(response ~ 
        s(Ufeito,bs="cr") + 
        s(Ufeito, SiteCode, bs="fs",xt=list(bs="cr")) + 
        s(SiteCode,bs="re"),
