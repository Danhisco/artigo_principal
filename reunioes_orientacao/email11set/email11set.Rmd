...
#             hide_code=TRUE)
update_file(file="reunioes_orientacao/email11set/email11set.Rmd",
            gpath="mestrado/reunioes_orientacao/",
            hide_code=TRUE)
# overwrite the current Rmd with the current google drive 
download_file(file="reunioes_orientacao/email11set/email11set.Rmd",
              gpath="mestrado/reunioes_orientacao/")
```
...
adply(l_f,1,\(s) sub("cbind\\(nCong,100-nCong\\) ","",s) %>% 
        ...
        gsub(",xt\\=list\\(bs\\=\\'cr\\'\\)","",.)) %>%
  select(-X1) %>% 
  rename(`modelos para descrever cbind(nCong,100-nCong)` = V1)
```

Esses formulas foram aplicadas a
