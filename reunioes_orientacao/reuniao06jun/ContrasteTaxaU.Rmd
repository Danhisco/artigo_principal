...
l_tabsel <- lapply(l_md,f_TabSelGAMM)
l_tabsel
l_md_maisplausivel <- lapply(names(l_md),\(li){
  df_tabsel <- l_tabsel[[li]]
  lmd <- l_md[[li]]
...
```{r tabela de seleção dos modelos para área}
l_tabsel[["area"]]
```
```{r diagnóstico modelo mais plausível área}
i <- 1
...
```{r tabela de seleção dos modelos para frag.perse}
l_tabsel[["frag.perse"]]
```
```{r}
i <- 2
...
```{r tabela de seleção dos modelos para frag.total}
l_tabsel[["frag.total"]]
```
```{r}
i <- 3
